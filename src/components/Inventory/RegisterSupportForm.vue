<template>
	<div class="support-form-shell">
		<NcNoteCard v-if="supportSuccess" type="success" class="support-success">
			<strong>{{ t('employees', 'Support was registered successfully.') }}</strong>
			<span>{{ t('employees', 'Recorded time: {duration}', { duration: formattedSuccessDuration }) }}</span>
			<span>{{ t('employees', 'The time was added to your reports as a non-billable activity.') }}</span>
			<a :href="supportedDeviceUrl">{{ t('employees', 'View device') }}</a>
		</NcNoteCard>
		...
		<form v-else @submit.prevent="submitSupport">
			<section aria-labelledby="support-device-heading">
				...
				<div v-if="!selectedDevice" class="support-search">
					<NcTextField
						ref="searchField"
						:value.sync="search"
						:label="t('employees', 'Search device')"
						:disabled="submitting"
					...
					<div v-else-if="results.length" class="support-results">
						<button v-for="device in results"
							...
							<Laptop :size="20" />
							<span><strong>{{ device.device_name || device.system_name || t('employees', 'Device') }}</strong><small>{{ deviceSecondary(device) }}</small></span>
						</button>
					</div>
				</div>
				...
				<div v-else class="support-device-card">
					<div class="support-device-main">
						<NcAvatar v-if="selectedDevice.employee_uid"
							:user="selectedDevice.employee_uid"
						...
						<div><strong>{{ selectedDevice.device_name || selectedDevice.system_name }}</strong><small>{{ employeeName }}</small></div>
						<NcButton type="tertiary"
							:aria-label="t('employees', 'Clear selected device')"
							:disabled="submitting"
							@click="clearDevice">
							<template #icon>
								<Close :size="20" />
					...
					<dl>
						<div v-if="selectedDevice.serial_number">
							<dt>{{ t('employees', 'Serial number') }}</dt><dd>{{ selectedDevice.serial_number }}</dd>
						</div>
						<div v-if="modelName">
			...
			<section aria-labelledby="support-classification-heading">
				<h3 id="support-classification-heading">
					{{ t('employees', 'Classification') }}
				</h3>
				<div class="support-fields">
					<label><span>{{ t('employees', 'Category') }}</span><select v-model="form.categoria" :disabled="submitting"><option value="mantenimiento">{{ t('employees', 'Maintenance') }}</option><option value="reparacion">{{ t('employees', 'Repair') }}</option><option value="diagnostico">{{ t('employees', 'Diagnostics') }}</option><option value="configuracion">{{ t('employees', 'Configuration') }}</option><option value="otro">{{ t('employees', 'Other') }}</option></select></label>
					<label><span>{{ t('employees', 'Priority') }}</span><select v-model="form.priority" :disabled="submitting"><option value="baja">{{ t('employees', 'Low') }}</option><option value="media">{{ t('employees', 'Medium') }}</option><option value="alta">{{ t('employees', 'High') }}</option><option value="critica">{{ t('employees', 'Critical') }}</option></select></label>
				</div>
			</section>

			<section aria-labelledby="support-duration-heading">
				<h3 id="support-duration-heading">
					{{ t('employees', 'Time spent') }}
				</h3>
				<div class="support-time-grid">
					<SupportDurationFields v-model="form.duration_minutes" :disabled="submitting" />
					<label class="support-date"><span>{{ t('employees', 'Support date') }}</span><input v-model="form.date" type="datetime-local" :disabled="submitting"></label>
				</div>
			</section>
			...
			<section aria-labelledby="support-description-heading">
				...
				<NcTextArea :value.sync="form.description" :label="t('employees', 'Description')" :disabled="submitting" />
			</section>
			...
			<div class="support-actions">
				...
				<NcButton type="primary" native-type="submit" :disabled="!formValid">
					...
					</template>{{ submitting ? t('employees', 'Registering') : t('employees', 'Register support') }}
				</NcButton>
			</div>
		</form>
	</div>
...
function initialForm() {
	return { categoria: 'mantenimiento', priority: 'media', description: '', date: localDateTimeValue(), duration_minutes: null }
}

export default {
	name: 'RegisterSupportForm',
	...
	computed: {
		employeeName() { return this.selectedDevice?.empleado_displayname || this.selectedDevice?.employee_uid || t('employees', 'Unassigned') },
		modelName() { return [this.selectedDevice?.brand, this.selectedDevice?.model].filter(Boolean).join(' ') },
		responsibleName() { const user = getCurrentUser(); return user?.displayName || user?.uid || t('employees', 'Current user') },
		formValid() { return this.selectedDevice !== null && this.form.categoria !== '' && this.form.priority !== '' && this.form.description.trim() !== '' && Number.isInteger(this.form.duration_minutes) && this.form.duration_minutes > 0 && isValidSupportDate(this.form.date) && !this.submitting },
		formattedSuccessDuration() { return formatSupportDuration(this.supportSuccess?.duration_minutes) },
		supportedDeviceUrl() { return this.supportSuccess?.id_team ? `${generateUrl('/apps/employees/')}#/Inventory?deviceId=${encodeURIComponent(this.supportSuccess.id_team)}` : '' },
	},
	...
	methods: {
		...
		deviceSecondary(device) { return [device.system_name, device.serial_number, device.empleado_displayname || device.employee_uid].filter(Boolean).join(' · ') },
		...
		async submitSupport() {
			...
			const payload = { id_team: Number(this.selectedDevice.id_team), categoria: this.form.categoria, priority: this.form.priority, details: this.form.description.trim(), date: this.form.date, duration_minutes: this.form.duration_minutes }
			try {
				const response = await inventoryService.createSupport(payload)
				const result = response?.data ?? response
				this.supportSuccess = { id_support: Number(result?.id_support), id_report: Number(result?.id_report), id_team: Number(result?.id_team || payload.id_team), duration_minutes: Number(result?.duration_minutes || payload.duration_minutes), duplicate: Boolean(result?.duplicate) }
				this.resetAfterSuccess()
				showSuccess(t('employees', 'Support was registered successfully.'))
				this.$emit('success', this.supportSuccess)
			} catch (error) {
				const data = error?.response?.data?.ocs?.data ?? error?.response?.data
				showError(data?.message || t('employees', 'Support could not be registered.'))
