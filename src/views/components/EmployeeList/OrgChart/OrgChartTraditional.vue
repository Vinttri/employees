<template>
	<div class="traditional-organigram">
		<NcEmptyContent v-if="!organizationTree"
			...
			:name="t('employees', 'No employees to display.')"
			:description="t('employees', 'There is no valid employee information available for the organization chart.')">
			<template #icon>
				...
				<article v-else class="employee-node">
					...
					<div class="employee-node__identity">
						...
						<span class="employee-node__level">
							{{ t('employees', 'Level {n}', { n: nodeData.depth }) }}
						</span>
					</div>

					<span v-if="nodeData.directCount"
						class="employee-node__reports"
						:title="t('employees', '{count} direct reports', { count: nodeData.directCount })">
	...
	methods: {
		...
		buildEmployeeMap() {
			...
			this.employees.forEach(employee => {
				const employeeId = this.normalizeEmployeeId(employee?.id_employees)
		...
		buildParentMap(employeesById) {
			...
			this.getOrderedRelations().forEach(relation => {
				const managerId = this.normalizeEmployeeId(relation?.id_employee)
				const dependentId = this.normalizeEmployeeId(relation?.id_dependent)
		...
		buildTreeNode(employeeId, depth, branchVisited, context) {
			...
			return {
				...
				uid,
				name: displayName || uid || t('employees', 'Unknown employee'),
				depth,
		...
		warnHierarchy(message, details) {
			...
			console.warn(`[Employee] ${message}`, details)
		},
