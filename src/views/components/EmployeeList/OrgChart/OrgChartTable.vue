	...
	<div class="organigrama-table-wrapper">
		<div class="organigrama-table-toolbar">
			...
			<span class="organigrama-table-count">
				{{ t('employees', '{count} managers', { count: filteredManagers.length }) }}
			</span>
		</div>

		<div v-if="filteredManagers.length === 0" class="organigrama-table-empty">
			{{ t('employees', 'No hierarchical relationships found yet.') }}
		</div>
				...
				<div class="manager-card__header" @click="toggle(manager.id)">
					...
					<div class="manager-card__stats">
						<span class="stat-pill stat-pill--direct">
							{{ manager.directCount }} {{ t('employees', 'direct') }}
						</span>
						<span class="stat-pill stat-pill--total">
							{{ manager.totalCount }} {{ t('employees', 'total') }}
						</span>
					</div>
				</div>
						...
						<table class="dependents-table">
							<thead>
								<tr>
									...
									<th>{{ t('employees', 'Level') }}</th>
									<th>{{ t('employees', 'Reports through') }}</th>
								</tr>
							</thead>
							<tbody>
								<tr
...
export default {
	...
	computed: {
		...
		childrenMap() {
			...
			this.relaciones.forEach(rel => {
				if (!map[rel.id_employee]) map[rel.id_employee] = []
				map[rel.id_employee].push(rel.id_dependent)
			})
			return map
		},
	...
	methods: {
		...
		levelLabel(depth) {
			const labels = {
				1: this.t('employees', 'Level 1'),
				2: this.t('employees', 'Level 2'),
				3: this.t('employees', 'Level 3'),
			}
			return labels[depth] || this.t('employees', 'Level {n}', { n: depth })
		},
			...
			const walk = (currentId, depth, pathNames) => {
				...
				childrenIds.forEach(childId => {
					...
					result.push({
						...
						depth,
						pathLabel: depth > 1
							? this.t('employees', 'via {chain}', { chain: pathNames.join(' › ') })
							: '',
					})
