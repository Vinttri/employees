<template>
	<div class="contacts-list__item-wrapper">
			...
			<div class="container-search-profile">
				<div class="button-container-profile">
					<NcActions>
						...
						<NcActionButton @click="DeactiveUserDialog(data.id_employees)">
							<template #icon>
								<AccountEdit :size="20" />
							</template>
		...
		<NcDialog
			...
			:message="t('employees', 'Are you sure you want to disable the account?')"
			:buttons="buttons" />
...
<script>
import OrgChartNetwork from './OrgChart/OrgChartNetwork.vue'
...
export default {
	...
	data() {
		return {
			...
			avatarVersion: 0,
			buttons: [
				{ label: this.t('employees', 'OK'), type: 'primary', callback: () => this.DeactiveUser() },
			],
		}
	},
	computed: {
	...
	methods: {
		...
		async DeactiveUser() {
			try {
				await axios.post(generateUrl('/apps/employees/DesactivarEmpleado'), {
					id_employees: this.SelectedEmpleado,
				})
		...
		async handleCroppedImage(blob) {
			...
			try {
				const response = await axios.post(generateUrl('/apps/employees/uploadAvatar'), formData, {
					headers: { 'Content-Type': 'multipart/form-data' },
				})
				this.avatarVersion = response.data.version
				this.refreshAvatar()
			} catch (err) {
				showError(this.t('employees', 'Error uploading image.'))
			}
		},
