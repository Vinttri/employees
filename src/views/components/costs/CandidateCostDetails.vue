	...
	<div class="candidate-detail">
			...
			<dl class="candidate-detail__headline">
				<div>
					<dt>
						...
						<HelpHint
							:label="t('employees', 'About estimated fit')"
							:text="t('employees', 'Comparative indicator based on availability, related experience, previous work with the company, billable history and relative cost. It does not make the final decision.')" />
					</dt>
					<dd>{{ percentValue(candidate.ajuste_estimado) }}</dd>
				</div>
				<div>
					<dt>
						{{ t('employees', 'data quality') }}
						<HelpHint
							:label="t('employees', 'About data quality')"
							:text="t('employees', 'Indicates how complete the information used in this analysis is. It does not evaluate employee performance.')" />
					</dt>
					<dd>
		...
		<div class="candidate-detail__sections">
			<section class="candidate-detail__section">
				<h3>
					{{ t('employees', 'Capacity and availability') }}
					<HelpHint
				...
				<p v-if="candidate.capacidad_calculable !== true" class="candidate-detail__notice">
					{{ t('employees', 'Daily reference hours are not configured, so availability cannot be calculated.') }}
				</p>
			...
			<section class="candidate-detail__section">
				<h3>{{ t('employees', 'Related experience') }}</h3>
				<p class="candidate-detail__context">
					{{ t('employees', '{count} selected activities', { count: selectedActivityIds.length }) }}
				</p>
			...
			<section class="candidate-detail__section">
				<h3>
					{{ t('employees', 'Cost and billable history') }}
					<HelpHint
						:label="t('employees', 'About estimated cost')"
						:text="t('employees', 'Hourly cost is used only to estimate the temporary scenario cost.')" />
				</h3>
				<dl class="candidate-detail__facts">
			...
			<section class="candidate-detail__section">
				<h3>{{ t('employees', 'Estimated fit breakdown') }}</h3>
				<dl class="candidate-detail__facts">
			...
			<section class="candidate-detail__section candidate-detail__section--strengths">
				<h3>{{ t('employees', 'Supporting factors') }}</h3>
				...
				<p v-else class="candidate-detail__empty">
					{{ t('employees', 'No supporting factors were identified.') }}
				</p>
			</section>

			<section class="candidate-detail__section candidate-detail__section--alerts">
				<h3>{{ t('employees', 'Planning alerts') }}</h3>
		...
		<p v-if="projectFullyAssigned && !alreadySelected" class="candidate-detail__disabled-reason">
			{{ t('employees', 'Project hours are fully assigned. You can still review candidate details.') }}
		</p>
	</div>
</template>
...
export default {
	...
	computed: {
		...
		hasRelatedExperience() {
			return [
				...
				|| Boolean(this.experience.last_company_report)
		},
		adjustmentBreakdown() {
			const breakdown = this.candidate.desglose_ajuste || {}
			return [
				{ key: 'disponibilidad', label: t('employees', 'Estimated availability') },
				{ key: 'experiencia_actividades', label: t('employees', 'Experience in activities') },
				{ key: 'experiencia_empresa', label: t('employees', 'Experience with company') },
				{ key: 'cargabilidad', label: t('employees', 'Historical billable percentage') },
				{ key: 'costo', label: t('employees', 'Relative cost') },
			].map(factor => ({
				...factor,
				value: this.hasNumber(breakdown[factor.key])
					? t('employees', '{value} points', { value: this.number(breakdown[factor.key]) })
					: t('employees', 'Not available'),
			}))
		},
		translatedStrengths() {
	...
	methods: {
		...
		strengthLabels() {
			return {
				experiencia_reciente_actividades: t('employees', 'Has recent experience in the selected activities.'),
				experiencia_actividades: t('employees', 'Has recorded experience in the selected activities.'),
				experiencia_empresa: t('employees', 'Has previously worked with this company.'),
				capacidad_suficiente: t('employees', 'Has sufficient estimated capacity for the requirement.'),
				disponibilidad_suficiente: t('employees', 'Has sufficient estimated availability for the requirement.'),
				cargabilidad_alta: t('employees', 'Has a strong historical billable percentage.'),
				costo_relativo_favorable: t('employees', 'Has a favorable relative cost within the visible candidate set.'),
			}
		},
		riskLabels() {
			return {
