<template>
	<section class="cost-quotation">
		<NcEmptyContent
			...
			:name="t('employees', 'No quote scenarios yet')"
			:description="t('employees', 'Create a planning scenario to estimate costs, profit and margin.')" />
		...
		<template v-else>
			<header class="quotation-toolbar">
				<div>
					...
					<h2>{{ t('employees', 'Quote scenarios') }}</h2>
				</div>

				<div class="scenario-tabs" role="tablist" :aria-label="t('employees', 'Quote scenarios')">
					<button
			...
			<div
				...
				<section
					...
					<h3 :id="`scenario-configuration-title-${activeSummary.key}`">
						{{ t('employees', 'Scenario configuration') }}
					</h3>
					<div class="scenario-actions">
						<label class="rename-field">
							<span>{{ t('employees', 'Scenario name') }}</span>
							<input
								type="text"
								:value="activeSummary.name"
								:aria-label="t('employees', 'Scenario name')"
						...
						<div class="scenario-action-buttons">
							<NcButton
								...
								:aria-label="t('employees', 'Duplicate scenario')"
							...
							<NcButton
								...
								:aria-label="t('employees', 'Delete scenario')"
					...
					<dl class="scenario-heading">
						<div class="heading-item">
							<dt>{{ t('employees', 'Company or group') }}</dt>
							<dd>{{ companyName(activeScenario) }}</dd>
						</div>
						<div class="heading-item">
							<dt>{{ t('employees', 'Period') }}</dt>
							<dd>{{ scenarioPeriod(activeScenario) }}</dd>
						</div>
						<div class="heading-item">
							<dt>{{ t('employees', 'Project leader') }}</dt>
				...
				<section class="team-section" :aria-labelledby="`team-title-${activeSummary.key}`">
					<div class="section-title">
						<div>
							...
							<p>{{ t('employees', 'Edit roles, assigned hours and activities for this temporary scenario.') }}</p>
						</div>
						<p class="team-hours-state" :class="{ 'team-hours-state--error': activeSummary.exceededHours > 0 }">
							...
							<span v-if="activeSummary.exceededHours > 0">
								· {{ t('employees', '{hours} over the requirement', {
									hours: formatHours(activeSummary.exceededHours),
								}) }}
							</span>
							<span v-else>
								· {{ t('employees', '{hours} remaining', {
						...
						<table class="team-table">
							<thead>
								<tr>
									...
									<th scope="col">
										{{ t('employees', 'Cost per hour') }}
									</th>
									<th scope="col">
										<span class="label-with-help">
											{{ t('employees', 'Estimated cost') }}
											<HelpHint
												:label="t('employees', 'About estimated cost')"
												:text="t('employees', 'Calculated by multiplying assigned hours by the hourly cost configured for the employee.')" />
										</span>
									</th>
									<th scope="col">
										<span class="label-with-help">
											{{ t('employees', 'Resulting occupancy') }}
											<HelpHint
												:label="t('employees', 'About estimated occupancy')"
												:text="t('employees', 'Percentage of effective capacity used by reported and assigned hours in this scenario.')" />
										</span>
									</th>
									<th scope="col">
										{{ t('employees', 'Alerts') }}
								...
								<tr v-for="(member, memberIndex) in activeSummary.team" :key="memberKey(member, memberIndex)">
									...
									<td :data-label="t('employees', 'Role')">
										...
										<input
											...
											@input="updateMember(memberIndex, 'role', $event.target.value)">
									</td>
									<td :data-label="t('employees', 'Activities')">
										<details v-if="scenarioActivities(activeScenario).length > 0" class="activity-picker">
											<summary>
												{{ t('employees', '{count} selected', { count: memberActivityCount(member) }) }}
											</summary>
									...
									<td :data-label="t('employees', 'Assigned hours')">
										<label class="visually-hidden" :for="`member-hours-${memberKey(member, memberIndex)}`">
											{{ t('employees', 'Assigned hours for {employee}', { employee: memberName(member) }) }}
										</label>
										<input
									...
									<td :data-label="t('employees', 'Estimated availability')">
										{{ formatNullableHours(memberAvailability(member)) }}
									</td>
									<td :data-label="t('employees', 'Cost per hour')">
										{{ formatNullableMoney(memberHourlyCost(member)) }}
									</td>
									<td :data-label="t('employees', 'Estimated cost')">
										{{ formatNullableMoney(memberEstimatedCost(member)) }}
									</td>
									<td :data-label="t('employees', 'Resulting occupancy')">
										{{ formatNullablePercentage(memberResultingOccupancy(member)) }}
									</td>
									<td :data-label="t('employees', 'Alerts')">
										...
										<span v-else class="status-inline status-inline--ok">
											...
											{{ t('employees', 'No alerts') }}
										</span>
									</td>
									<td :data-label="t('employees', 'Remove')">
										<NcButton
											type="tertiary-no-background"
											:aria-label="t('employees', 'Remove {employee} from scenario', { employee: memberName(member) })"
					...
					<NcEmptyContent
						v-else
						:name="t('employees', 'No employees in this scenario')"
						:description="t('employees', 'Add candidates from planning to estimate the team cost.')" />
				</section>
				...
				<section class="financial-summary" :aria-labelledby="`financial-title-${activeSummary.key}`">
					<div class="section-title">
						<div>
							<h3 :id="`financial-title-${activeSummary.key}`">
								{{ t('employees', 'Estimated totals') }}
							</h3>
							<p>{{ t('employees', 'These values are planning estimates, not final accounting results.') }}</p>
						</div>
					</div>
					...
					<div
						...
						<div>
							<strong>{{ t('employees', 'Cost calculation incomplete') }}</strong>
							<p>{{ t('employees', 'Complete the missing hourly costs to calculate reliable totals, profit and margin.') }}</p>
						</div>
					</div>
					<p v-else-if="activeSummary.price <= 0" class="financial-guidance">
						{{ t('employees', 'Enter a proposed price to calculate estimated profit and margin.') }}
					</p>

					<div class="financial-grid">
						<article class="metric-card">
							<span>{{ t('employees', 'Personnel cost') }}</span>
							<strong>{{ formatNullableMoney(activeSummary.personnelCost) }}</strong>
						</article>
						<article class="metric-card">
							<span class="label-with-help">
								{{ t('employees', 'Contingency amount') }}
								<HelpHint
									:label="t('employees', 'About contingency')"
									:text="t('employees', 'Additional percentage added to estimated personnel cost to account for uncertainty.')" />
							</span>
						...
						<article class="metric-card">
							<span class="label-with-help">
								{{ t('employees', 'Total estimated cost') }}
								<HelpHint
									:label="t('employees', 'About estimated cost')"
									:text="t('employees', 'Calculated from estimated personnel cost plus contingency.')" />
							</span>
							<strong>{{ formatNullableMoney(activeSummary.totalCost) }}</strong>
						</article>
						<article class="metric-card">
							<span class="label-with-help">
								{{ t('employees', 'Estimated profit') }}
								<HelpHint
									:label="t('employees', 'About estimated profit')"
									:text="t('employees', 'Proposed price minus estimated personnel cost and contingency.')" />
							</span>
							<strong>{{ formatNullableMoney(activeSummary.profit) }}</strong>
						</article>
						<article class="metric-card metric-card--margin">
							<span class="label-with-help">
								{{ t('employees', 'Estimated margin') }}
								<HelpHint
									:label="t('employees', 'About estimated margin')"
									:text="t('employees', 'Percentage of the proposed price remaining after estimated personnel cost and contingency.')" />
							</span>
					...
					<p class="margin-guidance">
						{{ t('employees', 'Margin states are guidance for this MVP: negative below 0%, low from 0% to 15%, and positive above 15%. They are not a formal business minimum.') }}
					</p>
				</section>
				...
				<section class="comparison-section" aria-labelledby="scenario-comparison-title">
					<div class="section-title">
						<div>
							<h3 id="scenario-comparison-title" class="label-with-help">
								{{ t('employees', 'Scenario comparison') }}
								<HelpHint
									:label="t('employees', 'About scenario comparison')"
									:text="t('employees', 'Compare temporary planning estimates. These scenarios are not saved when the page is reloaded.')" />
							</h3>
							<p>{{ t('employees', 'Compare up to three temporary alternatives without selecting one automatically.') }}</p>
						</div>
					</div>
					...
					<div
						...
						role="region"
						tabindex="0"
						:aria-label="t('employees', 'Scenario comparison')">
						<table class="comparison-table">
							...
							<tbody>
								...
								<tr>
									<th scope="row">
										{{ t('employees', 'Remaining estimated availability') }}
									</th>
									<td v-for="item in scenarioSummaries" :key="item.key">
								...
								<tr>
									<th scope="row">
										{{ t('employees', 'Average team experience') }}
									</th>
									<td v-for="item in scenarioSummaries" :key="item.key">
	...
	methods: {
		...
		scenarioName(scenario, index) {
			return scenario.name
				|| scenario.name
				|| t('employees', 'Scenario {letter}', { letter: String.fromCharCode(65 + index) })
		},
		...
		scenarioActivities(scenario) {
			...
			if (Array.isArray(scenario.Activity)) {
				return scenario.Activity
			}

			return Array.isArray(scenario.requerimiento?.Activity)
				? scenario.requerimiento.Activity
				: []
		},
		...
		marginStateLabel(state) {
			switch (state) {
			case 'positive':
				return t('employees', 'Positive margin')
			case 'low':
				return t('employees', 'Low margin')
			case 'negative':
				return t('employees', 'Negative margin')
			case 'incomplete':
				return t('employees', 'Incomplete cost information')
			default:
				return t('employees', 'No proposed price')
			}
		},
		memberSource(member) {
		...
		memberKey(member, index) {
			...
			return source.id_employee ?? source.id ?? source.uid ?? index
		},
		memberName(member) {
			const source = this.memberSource(member)
		...
		activityName(activity, index) {
			if (activity && typeof activity === 'object') {
				return activity.name
					|| activity.name
					|| t('employees', 'Activity {number}', { number: index + 1 })
			}
			return t('employees', 'Activity {number}', { number: index + 1 })
		},
		...
		memberRawAlerts(member) {
			...
			return alerts.map(alert => {
				if (alert && typeof alert === 'object') {
					return alert.key || alert.code || alert.type || 'review_assignment'
				}
				return String(alert)
			})
		},
		...
		alertLabel(alert) {
			...
			case 'horas_superan_disponibilidad':
				return t('employees', 'Assigned hours exceed estimated availability.')
			case 'occupation_over_100':
			case 'occupation_above_100':
			case 'ocupacion_supera_100':
				return t('employees', 'Resulting occupancy exceeds 100%.')
			case 'occupation_over_90':
			case 'occupation_above_90':
			case 'ocupacion_supera_90':
				return t('employees', 'Resulting occupancy exceeds 90%.')
			case 'no_activity_experience':
			case 'sin_experiencia_actividades':
				return t('employees', 'No related activity experience is recorded.')
			case 'sin_experiencia_empresa':
				return t('employees', 'No previous work with this company is recorded.')
			case 'missing_hourly_cost':
			case 'sin_costo_hora':
				return t('employees', 'Hourly cost is not configured.')
			case 'capacity_unavailable':
			case 'capacidad_no_calculable':
				return t('employees', 'Capacity cannot be calculated.')
			case 'datos_incompletos':
				return t('employees', 'Some information needed for the analysis is incomplete.')
			case 'required_hours_unassigned':
				return t('employees', 'Required hours exceed the hours currently assigned to the tentative team.')
			case 'assigned_hours_exceed_requirement':
				return t('employees', 'Assigned hours exceed the current project requirement. Adjust the team before continuing.')
			case 'single_person_dependency':
				return t('employees', 'The current plan depends entirely on one person.')
			case 'team_availability_shortfall':
				return t('employees', 'Required hours exceed the estimated availability of the tentative team.')
			case 'insufficient_available_staff':
				return t('employees', 'There may not be enough visible personnel with estimated availability for the requirement.')
			default:
				return t('employees', 'Review this assignment.')
			}
		},
		...
		companyName(scenario) {
			...
			if (company && typeof company === 'object') {
				return company.name || company.name || t('employees', 'Not selected')
			}
			return company || scenario.nombre_empresa || t('employees', 'Not selected')
		},
		leaderName(scenario) {
			...
			if (leader && typeof leader === 'object') {
				return leader.displayname
					...
					|| leader.name
					|| leader.uid
					|| t('employees', 'Not selected')
			}
			return scenario.nombre_lider || t('employees', 'Not selected')
		},
		scenarioPeriod(scenario) {
			const start = scenario.startDate
				?? scenario.date_start
				?? scenario.periodo?.date_start
			const end = scenario.endDate
				?? scenario.date_end
				?? scenario.periodo?.date_end
			if (!start || !end) {
				return t('employees', 'Not selected')
			}
		...
		formatHours(value) {
			return t('employees', '{hours} h', { hours: this.formatNumber(value) })
		},
		...
		formatNullablePercentage(value) {
			return value === null || value === undefined
				? '—'
				: t('employees', '{percentage}%', { percentage: this.formatNumber(value) })
		},
		...
		updateMemberHours(index, value) {
			...
			if (hours === null || hours <= 0) {
				this.$set(
					this.memberHourErrors,
					index,
					t('employees', 'Assigned hours must be greater than zero.'),
				)
				return
			}
			if (hours > maximum) {
		...
		toggleMemberActivity(memberIndex, activity, checked) {
			...
			this.updateMember(memberIndex, 'Activity', activities)
		},
