<template>
	<AppContentList class="content-list">
			...
			<div class="search-contacts-field">
				<div class="container-search">
					<div class="input-container">
						<input v-model="query" type="text" :placeholder="t('employees', 'Search positions...')">
					</div>
					...
					<div class="button-container">
						<NcActions
							...
							<!--NcActionButton @click="showMessage('Delete')">
								...
								{{ t('employees', 'Export empty template') }}
							</NcActionButton-->
		...
		<VirtualList ref="scroller"
			class="contacts-list"
			data-key="id_positions"
			:data-sources="filteredList"
			:data-component="PositionsListItem"
			:estimate-size="60"
		...
		<NcModal
			v-if="modal"
			ref="modalRef"
			:name="t('employees', 'Add new Position')"
			@close="closeModal">
...
export default {
	...
	computed: {
		filteredList() {
			let Position = this.contacts
				.filter(item => this.matchSearch(item.name))
	...
	methods: {
		...
		matchSearch(Position) {
			if (this.query.trim() !== '') {
				return Position.toString().toLowerCase().search(this.query.trim().toLowerCase()) !== -1
			}
			return true
		},

		async getallsPositions() {
			try {
				await axios.get(generateUrl('/apps/employees/GetPositionsFix'))
					.then(
						(response) => {
		...
		Exportar() {
			this.toggle()
			axios.get(
				generateUrl('/apps/employees/ExportListPositions'),
				{
					responseType: 'blob',
				},
			).then(
		...
		async importar() {
			...
			try {
				await axios.post(generateUrl('/apps/employees/ImportListPositions'), formData,
					{
						headers: {
		...
		async crearPuesto() {
			try {
				await axios.post(generateUrl('/apps/employees/crearPuesto'),
					{
						name: this.nombre_area,
						level: this.nivel_area !== '' ? Number(this.nivel_area) : null,
					})
					.then(
						(response) => {
							showSuccess(t('employees', 'Position created successfully'))
							this.$root.$emit('reload')
							this.nombre_area = ''
							this.nivel_area = ''
							this.modal = false
