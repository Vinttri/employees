<template>
	<NcAppContent :name="t('employees', 'Maintenance calendar')">
		<main class="page">
			<div v-if="!accessAllowed" class="state">
				<NcEmptyContent :name="t('employees', 'Access denied')" :description="t('employees', 'You do not have permission to view maintenance or the inventory module is disabled.')" />
			</div>
			<template v-else>
				<header>
					<div><h2>{{ t('employees', 'Maintenance calendar') }}</h2><p>{{ t('employees', 'Plan and review IT inventory maintenance campaigns.') }}</p></div><div class="header-actions">
						<NcButton @click="$router.push({ name: 'Inventory' })">
							{{ t('employees', 'Back to inventory') }}
						</NcButton><NcButton v-if="capabilities.canAdministerMaintenance" type="primary" @click="showForm = true">
							{{ t('employees', 'Schedule maintenance') }}
						</NcButton>
					</div>
				</header>
				<MaintenanceMetrics :groups="groups" :overdue="overdueTotal" />
				<section class="card">
					<MaintenanceFilters v-model="filters"
				...
				<section class="card calendar-card">
					<div v-if="loading" class="loading">
						<NcLoadingIcon :size="36" /><span>{{ t('employees', 'Loading maintenance campaigns') }}</span>
					</div><MaintenanceCalendar :load-events="loadGroups"
						:refresh-key="refreshKey"
						@open-group="openGroup"
						@load-error="handleError" />
				</section>
				<section class="card">
					<h3>{{ t('employees', 'Upcoming campaigns') }}</h3><ul v-if="upcoming.length" class="campaigns">
						<li v-for="group in upcoming" :key="group.id">
							<button type="button" @click="openGroup(group.id)">
								<strong>{{ group.title || group.department_name || t('employees', 'Maintenance campaign') }}</strong><span>{{ formatDateRange(group.periodStart || group.date_start || group.date_scheduled, group.periodEnd || group.date_end || group.date_scheduled) }} · {{ maintenanceTypeLabel(group.type, t) }}</span>
							</button>
						</li>
					</ul><NcEmptyContent v-else-if="!loading" :name="t('employees', 'No maintenance campaigns in this range')" />
				</section>
				<MaintenanceGroupForm v-if="showForm"
					:departments="departments"
					:technicians="technicians"
					:technicians-loading="techniciansLoading"
					:technicians-error="techniciansError"
...
import { formatDateRange, maintenanceCapabilities, maintenanceTypeLabel } from '../../../../utils/maintenanceFormatters.js'
import MaintenanceCalendar from './MaintenanceCalendar.vue'
import MaintenanceFilters from './MaintenanceFilters.vue'
import MaintenanceGroupForm from './MaintenanceGroupForm.vue'
import MaintenanceMetrics from './MaintenanceMetrics.vue'

export default {
	name: 'MaintenanceView',
	components: { NcAppContent, NcButton, NcEmptyContent, NcLoadingIcon, NcNoteCard, MaintenanceCalendar, MaintenanceFilters, MaintenanceGroupForm, MaintenanceMetrics },
	...
	computed: { capabilities() { return maintenanceCapabilities(this.permissions, this.Settings) }, accessAllowed() { return this.capabilities.moduleEnabled && this.capabilities.canViewMaintenance }, showTechnicianFilter() { return this.capabilities.canViewMaintenance && !this.capabilities.isTechnician }, upcoming() { return [...this.groups].sort((a, b) => String(a.periodStart || a.date_start || a.date_scheduled).localeCompare(String(b.periodStart || b.date_start || b.date_scheduled))).slice(0, 8) } },
	...
	methods: {
		...
		async loadTechnicians() { this.techniciansLoading = true; this.techniciansError = ''; try { this.technicians = await maintenanceService.getTechnicians() } catch (error) { this.technicians = []; this.techniciansError = t('employees', 'Could not load technicians.'); this.handleError(error) } finally { this.techniciansLoading = false } },
		...
		openGroup(id) { this.$router.push({ name: 'MaintenanceGroup', params: { id }, query: this.filterQuery() }) },
		groupCreated(data) { this.showForm = false; showSuccess(t('employees', 'Maintenance campaign created.')); const id = data?.group?.id || data?.id; if (id) this.openGroup(id); else { this.lastRequestKey = ''; this.refreshKey++ } },
		handleError(error) { if (error?.cancelled) return; this.error = error?.status === 401 ? t('employees', 'Your session has expired.') : error?.status === 403 ? t('employees', 'You do not have permission to perform this action.') : error?.status === 500 ? t('employees', 'An internal error occurred.') : error?.message || t('employees', 'Could not load maintenance information.'); showError(this.error) },
	},
}
</script>
