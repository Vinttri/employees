<template>
	<NcModal :name="t('employees', 'Schedule maintenance')" size="large" @close="close">
		<div class="form-modal">
			<h2>{{ t('employees', 'Schedule maintenance') }}</h2>
			<p class="step">
				{{ t('employees', 'Step {step} of 4', { step }) }}
			</p>
			<NcNoteCard v-if="errorMessage" type="error" :text="errorMessage" />

			<div v-if="step === 1" class="fields">
				<label>{{ t('employees', 'Title') }}<input v-model.trim="form.title" required></label>
				<label>{{ t('employees', 'Type') }}<select v-model="form.type"><option value="preventive">{{ t('employees', 'Preventive') }}</option><option value="corrective">{{ t('employees', 'Corrective') }}</option><option value="special">{{ t('employees', 'Special') }}</option></select></label>
				<label class="wide">{{ t('employees', 'Description') }}<textarea v-model.trim="form.description" rows="4" /></label>
			</div>

			<div v-if="step === 2">
				<div class="fields">
					<label>{{ t('employees', 'Department') }}<select v-model="form.departmentId" @change="departmentChanged"><option value="">{{ t('employees', 'Select a department') }}</option><option v-for="item in departments" :key="departmentId(item)" :value="departmentId(item)">{{ departmentName(item) }}</option></select></label>
					<label class="checkbox"><input v-model="form.includeDescendants" type="checkbox" @change="loadEquipment(0)">{{ t('employees', 'Include descendant departments') }}</label>
					<label>{{ t('employees', 'Search devices') }}<input v-model="equipmentSearch" type="search" @input="scheduleEquipmentSearch"></label>
				</div>
							...
							<tr>
								<th>
									<input type="checkbox"
										...
										:aria-label="t('employees', 'Select visible devices')"
										@change="toggleVisible($event.target.checked)">
								</th><th>{{ t('employees', 'Device') }}</th><th>{{ t('employees', 'Model') }}</th><th>{{ t('employees', 'Serial number') }}</th><th>{{ t('employees', 'Custodian') }}</th><th>{{ t('employees', 'Department') }}</th><th>{{ t('employees', 'Status') }}</th>
							</tr>
						</thead><tbody>
				...
				<div class="pagination">
					<span>{{ t('employees', '{count} devices selected', { count: selectedIds.length }) }}</span><NcButton :disabled="equipmentOffset === 0 || equipmentLoading" @click="loadEquipment(Math.max(0, equipmentOffset - equipmentLimit))">
			...
			<div v-if="step === 3" class="fields">
				<label>{{ t('employees', 'Period start') }}<input v-model="form.periodStart" type="date" required></label>
				<label>{{ t('employees', 'Period end') }}<input v-model="form.periodEnd"
					type="date"
					:min="form.periodStart || undefined"
					:max="periodEndMax || undefined"
					required></label>
				<label>{{ t('employees', 'Default start time') }}<input v-model="form.startTime" type="time"></label>
				<label>{{ t('employees', 'Default end time') }}<input v-model="form.endTime" type="time"></label>
				<label>{{ t('employees', 'Technician') }}<select v-model="form.technicianUid" :disabled="techniciansLoading"><option value="">{{ techniciansLoading ? t('employees', 'Loading technicians…') : t('employees', 'Unassigned') }}</option><option v-for="item in technicians" :key="technicianUid(item)" :value="technicianUid(item)">{{ technicianName(item) }}</option></select></label>
				<NcNoteCard v-if="techniciansError" type="error" :text="t('employees', 'Could not load technicians.')" />
				<NcNoteCard v-else-if="!techniciansLoading && !technicians.length" type="warning" :text="t('employees', 'There are no users with inventory technician permission. You can create the campaign unassigned and configure permissions later.')" />
			</div>

			<div v-if="step === 4" class="confirmation">
				<h3>{{ t('employees', 'Confirm campaign') }}</h3><dl><dt>{{ t('employees', 'Title') }}</dt><dd>{{ form.title }}</dd><dt>{{ t('employees', 'Type') }}</dt><dd>{{ form.type }}</dd><dt>{{ t('employees', 'Department') }}</dt><dd>{{ selectedDepartmentName }}</dd><dt>{{ t('employees', 'Campaign period') }}</dt><dd>{{ formatDateRange(form.periodStart, form.periodEnd) }}</dd><dt>{{ t('employees', 'Default schedule') }}</dt><dd>{{ formatOptionalTimeRange(form.startTime, form.endTime, t('employees', 'No schedule defined')) }}</dd><dt>{{ t('employees', 'Technician') }}</dt><dd>{{ selectedTechnicianName || t('employees', 'Unassigned') }}</dd><dt>{{ t('employees', 'Devices') }}</dt><dd>{{ selectedIds.length }}</dd></dl>
				<NcNoteCard v-if="!form.technicianUid" type="warning" :text="t('employees', 'This campaign will be created without an assigned technician.')" />
			</div>

			<div class="actions">
				<NcButton :disabled="busy" @click="step === 1 ? close() : step--">
					{{ step === 1 ? t('employees', 'Cancel') : t('employees', 'Previous') }}
					...
					:disabled="busy"
					@click="submit(false)">
					<NcLoadingIcon v-if="busy" :size="20" />{{ t('employees', 'Create campaign') }}
				</NcButton>
			</div>
			<MaintenanceConflictDialog v-if="conflicts"
				:conflicts="conflicts"
				:busy="busy"
...
export default {
	...
	components: { NcButton, NcEmptyContent, NcLoadingIcon, NcModal, NcNoteCard, MaintenanceConflictDialog },
	...
	methods: {
		...
		async loadEquipment(offset = 0) { if (!this.form.departmentId) { this.equipment = []; this.equipmentTotal = 0; return } this.equipmentLoading = true; try { const data = await maintenanceService.getEligibleEquipment(this.form.departmentId, { includeDescendants: this.form.includeDescendants, includeInactive: false, search: this.equipmentSearch.trim() || undefined, limit: this.equipmentLimit, offset }); this.equipment = data.items || []; this.equipmentTotal = Number(data.total || 0); this.equipmentOffset = offset } catch (error) { this.errorMessage = error.status === 500 ? t('employees', 'An internal error occurred.') : error.message } finally { this.equipmentLoading = false } },
		...
		next() { this.errorMessage = ''; if (this.step === 1 && !this.form.title) return this.fail(t('employees', 'Title is required.')); if (this.step === 2 && (!this.form.departmentId || !this.selectedIds.length)) return this.fail(t('employees', 'Select a department and at least one device.')); if (this.step === 3) { if (!this.form.periodStart || !this.form.periodEnd) return this.fail(t('employees', 'Enter a valid campaign period.')); if (this.form.periodEnd < this.form.periodStart) return this.fail(t('employees', 'The end date cannot be earlier than the start date.')); if (this.periodEndMax && this.form.periodEnd > this.periodEndMax) return this.fail(t('employees', 'The campaign period is too long.')); if ((this.form.startTime || this.form.endTime) && (!this.form.startTime || !this.form.endTime || this.form.endTime <= this.form.startTime)) return this.fail(t('employees', 'Enter a valid date and time range.')) } this.step++ },
		...
		async submit(allowPotentialDuplicates) { if (this.busy) return; this.busy = true; this.errorMessage = ''; try { const result = await maintenanceService.createGroup(this.payload(allowPotentialDuplicates)); this.$emit('created', result) } catch (error) { if (error.code === 'maintenance_duplicate_conflict') this.conflicts = error.conflicts; else this.errorMessage = error.status === 500 ? t('employees', 'An internal error occurred.') : error.message } finally { this.busy = false } },
	},
}
</script>
