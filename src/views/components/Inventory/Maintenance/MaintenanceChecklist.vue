<template>
	<section class="checklist">
		<div v-if="localItems.length">
			<div v-for="item in localItems" :key="item.code" class="checklist-row">
				<div>
					<strong>{{ item.label || item.code }}</strong>
					<span v-if="!editable">{{ resultLabel(item.result) }}</span>
				</div>
				<template v-if="editable">
					<select v-model="item.result" :disabled="saving" @change="changed(item)">
						<option v-for="option in options" :key="option.value" :value="option.value">
							{{ option.label }}
						</option>
					</select>
					<textarea v-model="item.observation"
						:disabled="saving"
						:placeholder="t('employees', 'Observation')"
						@input="changed(item)" />
					<p v-if="item.result === 'attention' && !String(item.observation || '').trim()" class="validation">
						{{ t('employees', 'Attention observation is required') }}
					</p>
				</template>
				<p v-else-if="item.observation" class="observation">
					{{ item.observation }}
				</p>
			</div>
			<div v-if="editable" class="checklist-actions">
				<span v-if="dirtyKeys.length">{{ t('employees', 'Unsaved changes') }}</span>
				<NcButton :disabled="saving || !dirtyKeys.length || !valid" @click="save">
					{{ saving ? t('employees', 'Saving…') : t('employees', 'Save checklist') }}
				</NcButton>
			</div>
		</div>
		<p v-else>
			{{ t('employees', 'Checklist information is not available for this record.') }}
		</p>
	</section>
</template>
...
export default {
	...
	computed: {
		options() {
			return [
				...
				{ value: 'ok', label: t('employees', 'Correct') },
				{ value: 'attention', label: t('employees', 'Requires attention') },
				{ value: 'not_applicable', label: t('employees', 'Not applicable') },
			]
		},
		valid() {
			return this.localItems.every(item => item.result !== 'attention' || String(item.observation || '').trim())
		},
	},
	watch: {
	...
	methods: {
		t,
		reset(items) {
			this.localItems = (items || []).map(item => ({ ...item, observation: item.observation || '' }))
			this.dirtyKeys = []
			this.$emit('dirty-change', false)
		},
		changed(item) {
			if (!this.dirtyKeys.includes(item.code)) this.dirtyKeys.push(item.code)
		...
		save() {
			...
			this.$emit('save', this.localItems.filter(item => dirty.has(item.code)).map(item => ({
				key: item.code,
				result: item.result,
