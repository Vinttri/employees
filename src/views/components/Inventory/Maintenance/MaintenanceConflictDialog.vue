				...
				<table>
					<thead><tr><th>{{ t('employees', 'Device') }}</th><th>{{ t('employees', 'Existing campaign') }}</th><th>{{ t('employees', 'Campaign period') }}</th><th>{{ t('employees', 'Type') }}</th><th>{{ t('employees', 'Status') }}</th></tr></thead><tbody>
						<tr v-for="(item, index) in conflicts" :key="item.id || index">
							<td>{{ item.team_identifier || item.equipmentId || item.id_team || '—' }}</td><td>{{ item.grupo_titulo || item.groupId || item.id_group || '—' }}</td><td>{{ formatDateRange(item.existingPeriodStart || item.date_scheduled, item.existingPeriodEnd || item.date_scheduled) }}</td><td>{{ item.type || item.type || '—' }}</td><td>{{ item.status || item.status || '—' }}</td>
						</tr>
					</tbody>
				</table>
...
import { formatDateRange } from '../../../../utils/maintenanceFormatters.js'
export default { name: 'MaintenanceConflictDialog', components: { NcButton, NcModal }, props: { conflicts: { type: Array, default: () => [] }, busy: { type: Boolean, default: false } }, methods: { t, formatDateRange, confirmOverride() { if (window.confirm(t('employees', 'Create the campaign despite potential duplicates?'))) this.$emit('confirm') } } }
</script>
