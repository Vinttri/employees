<template>
	<NcAppContent :name="t('employees', 'Companies and groups')">
		<div class="companies-page">
			<List :loading="loading"
				:listas="filteredListas"
				:select="select"
				:show-options="canAdminCustomers"
				:show-toggle-status="canAdminCustomers"
				:toggle-status-label="selectedIsActive ? t('employees', 'Disable') : t('employees', 'Enable')"
					...
					<div class="empty">
						<div class="areas-empty-state">
							<div class="areas-empty-card">
								...
								<h2>{{ t('employees', 'Companies and groups') }}</h2>
								<h1>{{ t('employees', 'Select a client for more details') }}</h1>

								<p class="areas-empty-description">
									{{ t('employees', 'Choose a client, company or group from the list to view its information, assigned collaborators, service fees or edit its details.') }}
								</p>
								...
								<div class="stats-grid">
									<div class="stat-card">
										...
										<div>
											<span>{{ t('employees', 'Total records') }}</span>
									...
									<div class="stat-card">
										...
										<div>
											<span>{{ t('employees', 'Main groups') }}</span>
									...
									<div class="stat-card">
										...
										<div>
											<span>{{ t('employees', 'Sub-companies') }}</span>
				...
				<template #custombuttons>
					<NcActions :open="button" @click="toggle">
						...
						<NcActionInput v-model="sortOrder"
							...
							:manual-open="true"
							:options="[
								{ label: t('employees', 'A to Z'), value: 'az' },
								{ label: t('employees', 'Z to A'), value: 'za' },
							]">
							{{ t('employees', 'Sort') }}
						</NcActionInput>

						<NcActionCheckbox v-model="onlyParents">
							{{ t('employees', 'Only Main Groups') }}
						</NcActionCheckbox>
						<NcActionCheckbox v-model="onlySpecial">
							{{ t('employees', 'Only Special Clients') }}
						</NcActionCheckbox>
						<NcActionCheckbox v-model="showDisabled">
							{{ t('employees', 'Show disabled') }}
						</NcActionCheckbox>
						<NcActionCheckbox v-model="onlyDisabled">
							{{ t('employees', 'Only Disabled') }}
						</NcActionCheckbox>
					...
					<div class="client-details">
						<div>
							<div class="details-header"
								:class="{ 'details-header--special': selectedClient.special }">
								...
								<div class="details-title">
									...
									<h2>{{ selectedClient.name || t('employees', 'Without name') }}</h2>
									<p>{{ selectedClient.details || t('employees', 'No description available.') }}</p>
								</div>
							</div>
							...
							<VueTabs v-model="activeCompanyTab"
								...
								type="grow">
								<VTab :title="t('employees', 'General Information')">
									<div class="btn-top">
										<div class="info-section">
											<div class="section-head">
												<div>
													<p class="section-label">
														{{ t('employees', 'Company Information') }}
													</p>
												</div>
											</div>
											...
											<div class="info-grid">
												<div class="detail-card">
													<span>{{ t('employees', 'Legal Business Name') }}</span>
													<span class="value-text">{{ selectedClient.legal_name || '-'
													}}</span>
												</div>

												<div class="detail-card">
													<span>{{ t('employees', 'Project Manager') }}</span>
												...
												<div class="detail-card">
													<span>{{ t('employees', 'Primary Contact') }}</span>
													<span class="value-text">{{ selectedClient.name_contact || '-'
													}}</span>
												</div>

												<div class="detail-card">
													<span>{{ t('employees', 'Phone Number') }}</span>
													<span class="value-text">{{ selectedClient.phone || '-' }}</span>
												</div>

												<div class="detail-card">
													<span>{{ t('employees', 'Email Address') }}</span>
													<span class="value-text">{{ selectedClient.email || '-' }}</span>
												</div>

												<div class="detail-card">
													<span>{{ t('employees', 'Location') }}</span>
													<span class="value-text">{{ selectedClient.location || '-'
													}}</span>
												</div>

												<div class="detail-card">
													<span>{{ t('employees', 'Special Client') }}</span>
													<span class="value-text">{{ Number(selectedClient.special) ?
														t('employees',
															'Yes') : t('employees', 'No') }}</span>
												</div>

												<div class="detail-card">
													<span>{{ t('employees', 'Status') }}</span>
													<span class="value-text">{{ Number(selectedClient.status) ?
														t('employees',
															'Active') : t('employees', 'Inactive') }}</span>
												</div>
											</div>
										</div>
									</div>
									...
									<div class="btn-top">
										<div class="info-section">
											...
											<div class="details-grid">
												<div class="detail-card">
													<span>{{ t('employees', 'Parent group') }}</span>
												...
												<div class="detail-card detail-card-wide">
													<span>{{ t('employees', 'Hierarchy') }}</span>
													<div class="breadcrumb">
														<span>{{ parentName }}</span>
														<span class="separator">/</span>
														<span class="value-text">{{ selectedClient.name }}</span>
													</div>
												</div>
											</div>
											<!-- Sub-companies -->
											<div class="children-section">
												<div class="section-head">
													<div>
														<p class="section-label">
															{{ t('employees', 'Sub-companies') }}
														</p>
														<h3>{{ t('employees', 'Companies inside this group') }}</h3>
													</div>
												</div>
												...
												<div v-if="childCompanies.length > 0" class="children-grid">
													<button v-for="child in childCompanies"
														...
														<div class="child-info">
															<span class="value-text">{{ child.name }}</span>
															<span>{{ child.details || t('employees', 'No description available.')
															}}</span>
														</div>
												...
												<NcEmptyContent v-else
													:name="t('employees', 'No sub-companies')"
													:description="t('employees', 'This company or group does not have registered sub-companies.')">
													<template #icon>
														<OfficeBuilding />
													</template>
												</NcEmptyContent>
								...
								<VTab :title="t('employees', 'Fees')">
									<div class="info-section separator-top">
										<div class="section-head">
											<div>
												...
												<h3>{{ t('employees', 'Collaborators') }}</h3>
											</div>
										</div>
										...
										<div class="collaborators-block">
											...
											<div class="collaborator-row">
												<span class="collaborator-role">{{ t('employees', 'Project Leader') }}</span>
												...
												<span v-else class="value-text">
													{{ t('employees', 'No one has registered yet') }}
												</span>
											</div>

											<!-- Colaboradores -->
											<div class="collaborator-row">
												<span class="collaborator-role">{{ t('employees', 'Collaborators') }}</span>
									...
									<div class="info-section billing-section">
										<div class="section-head billing-head">
											<div class="billing-title">
												...
												<div>
													...
													<h3>{{ t('employees', 'Service Fees') }}</h3>
												</div>
											</div>
											<div class="billing-actions">
												<NcActions>
													...
													<NcActionButton v-if="canAdminCustomers" @click="toggleSelectMode">
														...
														{{ selectMode ? t('employees', 'Exit selection') : t('employees', 'Select') }}
													</NcActionButton>
												</NcActions>
										...
										<div v-if="selectMode" class="select-bar">
											<span>{{ selectedHonorarios.length }} {{ t('employees', 'selected') }}</span>
											<div class="select-bar__actions">
												...
												<NcButton type="primary"
													...
													{{ t('employees', 'Generate report') }}
												</NcButton>
											</div>
										</div>

										<NcEmptyContent v-if="!filteredHonorarios.length"
											:name="t('employees', 'No fees registered')"
											:description="t('employees', 'Register a service fee for this company.')">
											<template #icon>
												<OfficeBuilding />
											</template>
										</NcEmptyContent>

										<div v-else class="ProfessionalFee-list">
											<div v-for="honorario in filteredHonorarios"
												:key="honorario.id_fee"
												class="honorario-card"
												:class="{
													'honorario-special': Number(honorario.special) === 1,
													'honorario-card--selected': selectedHonorarios.includes(honorario.id_fee)
												}">
												<div class="honorario-header">
													<input v-if="selectMode"
														type="checkbox"
														:checked="selectedHonorarios.includes(honorario.id_fee)"
														class="honorario-checkbox"
														@change="toggleSeleccionHonorario(honorario.id_fee)">

													<div class="honorario-info">
														<span class="value-text">{{ honorario.service_type || t('employees',
															'Service') }}</span>
														<span class="honorario-date">
															{{ honorario.date_start }} — {{ honorario.date_end }}
														</span>
													</div>
													<div class="honorario-meta">
														<span class="honorario-amount">
															{{ formatImporte(montoAcumulado(honorario)) }} {{ honorario.type_currency }}
														</span>
														<span class="honorario-badge"
															:class="Number(honorario.active) ? 'badge-active' : 'badge-done'">
															{{ Number(honorario.active) ? t('employees', 'Active') : t('employees', 'Completed') }}
														</span>

														<!-- badge de type -->
														<span class="honorario-badge badge-type"
															:class="'badge-type-' + (honorario.type_fee || 'parcial')">
															{{ formatTipoHonorario(honorario.type_fee) }}
														</span>
														<!-- Botón reactivar — solo igualas completadas -->
														<div class="honorario-right-actions">
															<NcButton v-if="canAdminCustomers && Number(honorario.number_installments) === 0"
																type="secondary"
																@click="completarHonorarioBorrador(honorario)">
																{{ t('employees', 'Complete fee') }}
															</NcButton>

															<NcButton v-if="Number(honorario.number_installments) > 0"
																type="tertiary"
																@click="toggleParcialidades(honorario.id_fee)">
																{{ t('employees', 'Installments') }}
															</NcButton>
															...
															<NcActions v-if="canAdminCustomers" :force-menu="true">
																...
																<NcActionButton
																	v-if="honorario.type_fee === 'iguala' && Number(honorario.active) === 1"
																	@click="agregarParcialidadIguala(honorario.id_fee)">
																	<template #icon>
																...
																<NcActionButton
																	v-if="honorario.type_fee === 'iguala' && Number(honorario.active) === 1"
																	class="action-danger"
																	@click="askFinalizarHonorario(honorario.id_fee)">
																	<template #icon>
																		<CloseCircleOutline :size="20" />
																	</template>
																	{{ t('employees', 'Finalize') }}
																</NcActionButton>

																<NcActionButton
																	v-if="honorario.type_fee === 'iguala' && Number(honorario.active) === 0"
																	@click="reactivarHonorario(honorario.id_fee)">
																	<template #icon>
																		<Restore :size="20" />
																	</template>
																...
																<NcActionSeparator
																	v-if="honorario.type_fee === 'iguala'" />
																...
																<NcActionButton @click="askDeleteHonorario(honorario.id_fee)">
																	<template #icon>
												...
												<div v-if="parcialidadesAbiertas[honorario.id_fee]"
													class="parcialidades-list">
													<div class="parcialidades-head">
														<span>{{ t('employees', 'Installments') }}</span>
														<span>
															{{ (parcialidades[honorario.id_fee] || []).length }}
														</span>
													</div>
													<div v-if="loadingParcialidades[honorario.id_fee]"
														class="parcialidades-loading">
														{{ t('employees', 'Loading...') }}
													</div>
													<template v-else>
														<div v-for="p in (parcialidades[honorario.id_fee] || [])"
															:key="p.id_installment"
															class="parcialidad-row"
															:class="{
																'parcialidad-pagada': Number(p.paid) === 1,
																'parcialidad-facturada': Number(p.paid) === 2
															}">
															<div class="parcialidad-main">
																<div class="parcialidad-num-wrapper">
																	<span class="parcialidad-num">#{{ p.number_installment
																	}}</span>

																	<span
																		v-if="Number(p.paid) === 1 || Number(p.paid) === 2"
																		class="parcialidad-toggle"
																		:class="{ open: detalleAbierto[p.id_installment] }"
																		@click="toggleDetalleParcialidad(p.id_installment)">
																		<ChevronDown :size="16" />
																	</span>
																</div>
																<span class="parcialidad-fechas">{{ p.installment_start_date }} — {{
																	p.installment_end_date }}</span>
																<span class="parcialidad-importe">{{
																	formatImporte(p.amount_installment) }} {{
																	honorario.type_currency }}</span>

																<div class="parcialidad-actions">
																	<NcButton v-if="canAdminCustomers && Number(p.paid) === 0"
																		class="btn-pagar"
																		type="primary"
																		@click="abrirDialogPago(p.id_installment, honorario.id_fee)">
																		{{ t('employees', 'Mark as paid') }}
																	</NcButton>

																	<template v-else-if="canAdminCustomers && Number(p.paid) === 1">
																		<NcButton class="btn-factura"
																			type="secondary"
																			@click="confirmarFactura(p.id_installment, honorario.id_fee)">
																			{{ t('employees', 'Mark as invoiced') }}
																		</NcButton>
																	</template>

																	<template v-else-if="Number(p.paid) === 2">
																		<span class="parcialidad-completada">
																			{{ t('employees', 'Completed') }} ✓
																		</span>
																	</template>
																</div>
															</div>
															<div v-if="detalleAbierto[p.id_installment]" class="parcialidad-detalle">
																<span v-if="p.date_payment" class="parcialidad-detail-text">
																	💳 {{ t('employees', 'Paid') }}: {{ p.date_payment }}
																</span>

																<NcActions v-if="canAdminCustomers && Number(p.paid) === 1" class="parcialidad-detalle-actions">
																	<template #icon>
																		<DotsHorizontal :size="18" />
																	</template>
																	<NcActionButton @click="editarFechaPago(p, honorario.id_fee)">
																		<template #icon>
																			<PencilOutline :size="20" />
																		</template>
																		{{ t('employees', 'Edit payment date') }}
																	</NcActionButton>
																	<NcActionButton @click="askCancelarPago(p, honorario.id_fee)">
																		<template #icon>
																			<CloseCircleOutline :size="20" />
																		</template>
																		{{ t('employees', 'Cancel payment') }}
																	</NcActionButton>
																</NcActions>
															</div>
														</div>
						...
						<NcModal
							v-if="showPagoDialog"
							size="small"
							:name="t('employees', 'Register payment')"
							...
							<div class="payment-modal">
								...
								<h2>{{ t('employees', 'Register payment') }}</h2>
								<p class="payment-subtitle">
									{{ t('employees', 'Select the payment date for this installment.') }}
								</p>
								<div class="payment-field">
									<NcTextField
										v-model="fechaPago"
										type="date"
										:label="t('employees', 'Payment date')" />
								</div>
						...
						<NcDialog :open.sync="showCancelarPagoDialog"
							:name="t('employees', 'Confirm')"
							:message="t('employees', 'This will mark the installment as pending again. Continue?')"
							:buttons="[
								{ label: t('employees', 'Cancel'), callback: () => { showCancelarPagoDialog = false } },
								{ label: t('employees', 'Cancel payment'), type: 'primary', callback: () => { confirmarCancelarPago() } },
							]" />
						<NcDialog :open.sync="showFinalizarDialog"
							:name="t('employees', 'Finalize fee')"
							:message="t('employees', 'This will mark the fee as completed. No new installments will be generated. Continue?')"
							:buttons="[
								{ label: t('employees', 'Cancel'), callback: () => { showFinalizarDialog = false } },
								{ label: t('employees', 'Finalize'), type: 'primary', callback: () => { confirmarFinalizarHonorario() } },
							]" />
					</div>
				</template>
			</List>
		...
		<NcModal v-if="modal && canAdminCustomers"
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h2>{{ modalTitle }}</h2>
					<p>
						{{ t('employees', 'Register a main group or link the company to an existing parent group.') }}
					</p>
				</div>
				...
				<div class="form-grid">
					...
					<NcTextField required
						class="span-2"
						:value.sync="name"
						:label="t('employees', 'Company or group name')" />
					...
					<!-- legal_name -->
					<NcTextField :value.sync="legal_name" :label="t('employees', 'Business name')" />

					<!-- email -->
					<NcTextField :value.sync="email" :label="t('employees', 'Email')" />

					<!-- name_contact -->
					<NcTextField :value.sync="name_contact" :label="t('employees', 'Primary contact')" />

					<!-- phone -->
					<NcTextField :value.sync="phone" :label="t('employees', 'Phone number')" />

					<!-- location -->
					<NcTextField class="span-2" :value.sync="location" :label="t('employees', 'Location')" />
					<NcSelect v-model="project_leader"
						:input-label="t('employees', 'Project leader')"
						:options="projectManagers"
						:clearable="true"
						label="label"
						track-by="value" />
					<NcSelect v-model="collaborators"
						:options="collaboratorOptions"
						:multiple="true"
						label="label"
						track-by="value"
						:placeholder="t('employees', 'Collaborators')"
						class="aligned-select" />

					<!-- special -->
					<div class="special-client-card span-2">
						<NcCheckboxRadioSwitch v-model="special" type="switch" />
						<div class="special-client-info">
							<h3>{{ t('employees', 'Special Client') }}</h3>
							<p>
								{{ t('employees', 'Enable this option for special handling clients.') }}
							</p>
						</div>
					</div>
					...
					<!-- status -->
					<div class="special-client-card span-2">
						<NcCheckboxRadioSwitch v-model="status" type="switch" />
						<div class="special-client-info">
							<h3>{{ t('employees', 'Active') }}</h3>
							<p>
								{{ t('employees', 'Disable to deactivate this client without deleting it.') }}
							</p>
						</div>
					</div>
					...
					<NcSelect :key="options.length"
						v-model="client_parent"
						class="span-2"
						:input-label="t('employees', 'Parent group')"
					...
					<NcNoteCard type="info" class="span-2">
						{{ t('employees', 'Leave parent group empty to create a main group. Select a parent to create a sub-company.') }}
					</NcNoteCard>
				</div>
				...
				<div class="modal-actions">
					...
					<NcButton type="primary" :disabled="!isFormValid || saving" @click="save">
						{{ saving ? t('employees', 'Saving...') : saveLabel }}
					</NcButton>
				</div>
			</div>
		</NcModal>
		...
		<NcModal v-if="honorarioFilterModal"
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h2>{{ t('employees', 'Filter fees') }}</h2>
				</div>

				<div class="form-grid">
					<NcTextField class="span-2"
						:value.sync="hf_busqueda"
						:label="t('employees', 'Search by service name')" />
					...
					<NcSelect v-model="hf_tipo"
						...
						:placeholder="t('employees', 'Fee type')"
						label="label"
						track-by="value"
						:clearable="true" />
					...
					<div class="special-client-card span-2">
						...
						<div class="special-client-info">
							<h3>{{ t('employees', 'Special fees only') }}</h3>
						</div>
					</div>

					<div class="span-2">
						<span class="date-label">{{ t('employees', 'Date range') }}</span>
					</div>
					<NcTextField :value.sync="hf_desde" type="date" :label="t('employees', 'From')" />
					<NcTextField :value.sync="hf_hasta" type="date" :label="t('employees', 'To')" />
				</div>
		...
		<NcModal v-if="reporteModal && canAdminCustomers"
			size="normal"
			:name="t('employees', 'Generate report')"
			@close="reporteModal = false">
			<div class="modal-content">
				<div class="modal-header">
					<h2>{{ t('employees', 'Generate service fee report') }}</h2>
				</div>
				...
				<div class="form-grid">
					...
					<NcTextField class="span-2" :value.sync="rep_asunto" :label="t('employees', 'Subject')" />
				</div>
				...
				<div class="modal-actions">
					...
					<NcButton type="primary" :disabled="generandoReporte" @click="generarReporte">
						{{ generandoReporte ? t('employees', 'Generating...') : t('employees', 'Generate') }}
					</NcButton>
				</div>
			</div>
		</NcModal>
		...
		<NcModal v-if="honorarioModal && canAdminCustomers" :name="honorarioModalTitle" @close="closeHonorarioModal">
			<div class="modal-content">
				...
				<!-- Selector de type -->
				<div class="type-honorario-selector">
					<button v-for="type in tiposHonorario"
						:key="type.value"
						class="type-btn"
						:class="{ 'type-btn--active': h_tipo_honorario === type.value }"
						type="button"
						:disabled="isEditingHonorario"
						@click="!isEditingHonorario && (h_tipo_honorario = type.value)">
						<span class="type-icon">
							<span v-if="type.value === 'parcial'">📅</span>
							<span v-else-if="type.value === 'iguala'">🔄</span>
							<span v-else>⚡</span>
						</span>
						{{ type.label }}
					</button>
				...
				<NcNoteCard type="info" class="type-desc">
					<span v-if="h_tipo_honorario === 'parcial'">
						{{ t('employees', 'Fixed period. Installments are calculated automatically by month between start and end date.') }}
					</span>
					<span v-else-if="h_tipo_honorario === 'iguala'">
						{{ t('employees', 'Indefinite monthly fee. A new installment is generated each month. You can finalize it at any time.') }}
					</span>
					<span v-else>
						{{ t('employees', 'One-time fee. A single installment is created for the selected month.') }}
					</span>
				</NcNoteCard>

				<!-- Aviso de edición bloqueada -->
				<NcNoteCard v-if="isEditingHonorario" type="warning" class="type-desc">
					{{ t('employees', 'Dates and amount cannot be changed here to avoid regenerating installments. Only service, currency, title date and special flag can be edited.') }}
				</NcNoteCard>

				<div class="form-grid">
					<NcTextField :value.sync="h_tipo_servicio" :label="t('employees', 'Service type')" />

					<NcSelect v-model="h_titulo_mes"
						:options="meses"
						:placeholder="t('employees', 'Month')"
						label="label"
						track-by="value"
						:searchable="false" />
					<NcSelect v-model="h_titulo_anio"
						:options="anios"
						:placeholder="t('employees', 'Year')"
						label="label"
						track-by="value"
						:searchable="false" />
					...
					<NcTextField type="number"
						...
						:disabled="isEditingHonorario"
						:label="t('employees', 'Total amount')" />

					<div class="special-client-card span-2">
						<NcCheckboxRadioSwitch v-model="h_especial" type="switch" />
						<div class="special-client-info">
							<h3>{{ t('employees', 'Special fee') }}</h3>
							<p>{{ t('employees', 'Marks this service fee as special.') }}</p>
						</div>
					</div>

					<!-- Fecha inicio (todos los tipos) -->
					<div class="span-2">
						<span class="date-label">{{ t('employees', 'Start date') }}</span>
					</div>
					...
					<template v-if="h_tipo_honorario === 'parcial'">
						<div class="span-2">
							<span class="date-label">{{ t('employees', 'End date') }}</span>
						</div>
					...
					<NcNoteCard v-if="!isEditingHonorario && h_tipo_honorario === 'parcial' && h_numero_parcialidades > 0"
						type="info"
						class="span-2">
						{{ t('employees', '{n} installment(s) of {amount} {currency}', {
					...
					<NcNoteCard v-else-if="!isEditingHonorario && h_tipo_honorario === 'iguala'" type="info" class="span-2">
						{{ t('employees', 'Monthly fee of {amount} {currency} starting {mes} {anio}', {
					...
					<NcNoteCard v-else-if="!isEditingHonorario && h_tipo_honorario === 'eventual'" type="info" class="span-2">
						{{ t('employees', 'Single installment of {amount} {currency}', {
							amount: formatImporte(Number(h_importe_total || 0)),
							currency: h_tipo_moneda ? h_tipo_moneda.value : ''
						}) }}
					</NcNoteCard>
...
export default {
	...
	data() {
		return {
			activeCompanyTab: t('employees', 'Fees'),
			projectManagers: [],
			editing: false,
			saving: false,
			loading: true,
			...
			hf_estadoOptions: [
				...
				{ label: t('employees', 'Completed'), value: 'completado' },
			],
			...
			tiposHonorario: [
				{ label: t('employees', 'Installments'), value: 'parcial' },
				{ label: t('employees', 'Retainer fee'), value: 'iguala' },
				{ label: t('employees', 'One-time'), value: 'eventual' },
			],
	...
	computed: {
		canAdminCustomers() {
			return this.canSee('Client.admin')
		},
		...
		selectedClientType() {
			return Number(this.selectedClient?.client_parent || 0) === 0
				? t('employees', 'Main group')
				: t('employees', 'Sub-company')
		},

		selectedIsActive() {
			return Boolean(Number(this.selectedClient?.status ?? 1))
		},

		parentName() {
			const parentId = this.selectedClient?.client_parent

			if (!parentId || Number(parentId) === 0) {
				return t('employees', 'Main group')
			}

			return this.rawClients.find((client) => Number(client.id) === Number(parentId))?.name
		...
		childCompanies() {
			...
			return this.rawClients.filter((client) => {
				return Number(client.client_parent || 0) === Number(this.selectedClient.id)
			})
		},

		mainGroups() {
			return this.rawClients.filter((client) => Number(client.client_parent || 0) === 0 && Number(client.status ?? 1) === 1)
		},
		...
		activeClients() {
			return this.rawClients.filter((client) => Number(client.status ?? 1) === 1)
		},
		...
		modalTitle() {
			return this.editing
				? t('employees', 'Edit company or group')
				: t('employees', 'New company or group')
		},
		...
		filteredListas() {
			...
			if (!this.showDisabled) {
				data = data.filter(item => Number(item.status ?? 1) === 1)
			}

			if (this.onlyDisabled) {
				data = data.filter(item => Number(item.status ?? 1) === 0)
			}

			if (this.onlyParents) {
				data = data.filter(item =>
					Number(item.client_parent || 0) === 0,
				)
			}

			if (this.onlySpecial) {
				data = data.filter(item =>
					Number(item.special || 0) === 1,
				)
			}
		...
		projectManagerName() {
			return this.projectManagers.find(
				(emp) => Number(emp.value) === Number(this.selectedClient?.project_leader),
			)?.label || '-'
		},
		...
		selectedClientCollaborators() {
			const colabs = Array.isArray(this.selectedClient?.collaborators)
				? this.selectedClient.collaborators
				: []
		...
		collaboratorOptions() {
			...
			const leaderId = Number(this.project_leader.value ?? this.project_leader)
		...
		filteredHonorarios() {
			let data = [...this.ProfessionalFee]

			if (this.hf_busqueda.trim()) {
				const q = this.hf_busqueda.trim().toLowerCase()
				data = data.filter(h => (h.service_type || '').toLowerCase().includes(q))
			}

			if (this.hf_estado) {
				data = data.filter(h => {
					const active = Number(h.active) === 1
					return this.hf_estado.value === 'active' ? active : !active
				})
			}

			if (this.hf_tipo) {
				data = data.filter(h => (h.type_fee || 'parcial') === this.hf_tipo.value)
			}

			if (this.hf_soloEspecial) {
				data = data.filter(h => Number(h.special) === 1)
			}

			if (this.hf_desde) {
				data = data.filter(h => h.date_start >= this.hf_desde)
			}

			if (this.hf_hasta) {
				data = data.filter(h => h.date_end <= this.hf_hasta)
			}
		...
		honorarioModalTitle() {
			return this.editingHonorarioId
				? t('employees', 'Edit service fee')
				: t('employees', 'New service fee')
		},

		honorarioSaveLabel() {
			if (this.savingHonorario) {
				return t('employees', 'Saving...')
			}
			return this.editingHonorarioId
				? t('employees', 'Save changes')
				: t('employees', 'Create fee')
		},
	},
	...
	methods: {
		...
		async loadRequiredCustomerData() {
			try {
				...
				showError(
					t('employees', 'Error loading customer data: {error}', {
						error: String(err),
					}),
				)
			}
		...
		async GetAreasList() {
			try {
				const response = await axios.get(generateUrl('/apps/employees/GetAreasList'))
				...
				showError(t('employees', 'Error loading departments: {error}', { error: String(err) }))
			}
		},
		...
		formatTipoHonorario(type) {
			const labels = {
				parcial: t('employees', 'Installments'),
				iguala: t('employees', 'Retainer fee'),
				eventual: t('employees', 'One-time'),
			}

			return labels[type || 'parcial'] || type || '-'
		},

		montoAcumulado(honorario) {
			const lista = this.parcialidades[honorario.id_fee]

			if (Array.isArray(lista) && lista.length > 0) {
				return lista.reduce((sum, p) => sum + Number(p.amount_installment || 0), 0)
			}

			// Aún no se han cargado las parcialidades en el front: usar el
			// valor que ya trajo el backend, o el amount_total como último recurso.
			const acumuladoBackend = Number(honorario.monto_acumulado || 0)
			return acumuladoBackend > 0 ? acumuladoBackend : Number(honorario.amount_total || 0)
		},
		...
		async GetClientesEmpleadosLookup(showFailure = true) {
			try {
				const response = await axios.get(
					generateUrl('/apps/employees/GetClientesEmpleadosLookup'),
				)

				const Employee = response?.data?.ocs?.data || []

				this.projectManagers = Employee.map((employee) => ({
					value: Number(employee.id_employee),
					uid: employee.uid,
				...
				if (showFailure) {
					showError(
						t('employees', 'Error loading employees: {error}', {
							error: String(err),
						}),
					)
				} else {
		...
		async GetCompanieGroup(id) {
			try {
				const response = await axios.post(generateUrl('/apps/employees/GetCompanieGroup'), {
					id,
				})
				...
				showError(t('employees', 'Error loading company: {error}', { error: String(err) }))
			}
		},
		...
		async GetCompaniesGroups(showFailure = true) {
			...
			try {
				...
				this.options = []
				if (showFailure) {
					showError(t('employees', 'Error loading companies: {error}', { error: String(err) }))
				} else {
					throw err
				}
			} finally {
		...
		getPayload() {
			return {
				...
				legal_name: String(this.legal_name || '').trim(),
				project_leader: this.project_leader?.value ?? this.project_leader ?? null,
				collaborators: JSON.stringify(
					Array.isArray(this.collaborators)
						? this.collaborators.map(c => c.value ?? c)
						: [],
				),
				name_contact: String(this.name_contact || '').trim(),
				...
				client_parent: this.client_parent?.value ?? this.client_parent?.id ?? null,
		...
		async create() {
			if (!this.isFormValid) {
				showError(t('employees', 'Company or group name is required.'))
				return
			}
			...
			try {
				await axios.post(generateUrl('/apps/employees/crearCliente'), this.getPayload())

				showSuccess(t('employees', 'Company or group created successfully'))
				await this.GetCompaniesGroups()
				this.closeModal()
			} catch (err) {
				showError(t('employees', 'Error creating company: {error}', { error: String(err) }))
			} finally {
				this.saving = false
			}
		},
		...
		async delete() {
			if (!this.selectedClient?.id) {
				showError(t('employees', 'Select a company or group first.'))
				return
			}
			...
			try {
				await axios.post(generateUrl('/apps/employees/deleteCliente'), {
					id: this.selectedClient.id,
				})

				showSuccess(t('employees', 'Company or group deleted successfully'))
				this.select = []
				await this.GetCompaniesGroups()

			} catch (err) {
				showError(t('employees', 'Error deleting company: {error}', {
					error: String(err),
				}))
			} finally {
				this.loading = false
		...
		async modify() {
			...
			try {
				await axios.post(generateUrl('/apps/employees/modificarCliente'), {
					id: this.selectedClient.id,
					...this.getPayload(),
				})

				showSuccess(t('employees', 'Company or group updated successfully'))
				await this.GetCompanieGroup(this.selectedClient.id)
				await this.GetCompaniesGroups()
				this.closeModal()
			} catch (err) {
				showError(t('employees', 'Error updating company: {error}', { error: String(err) }))
			} finally {
				this.saving = false
			}
		},
		...
		edit() {
			...
			this.details = this.selectedClient.details || ''
			this.legal_name = this.selectedClient.legal_name || ''
			this.name_contact = this.selectedClient.name_contact || ''
			this.phone = this.selectedClient.phone || ''
			this.email = this.selectedClient.email || ''
			this.location = this.selectedClient.location || ''
			this.special = Boolean(Number(this.selectedClient.special))
			this.status = Boolean(Number(this.selectedClient.status ?? 1))

			this.project_leader = this.projectManagers.find(
				(emp) => Number(emp.value) === Number(this.selectedClient.project_leader),
			) || null

			// collaborators ya viene como array de ids desde el mapper
			const colabs = Array.isArray(this.selectedClient.collaborators)
				? this.selectedClient.collaborators
				: []

			this.collaborators = this.projectManagers.filter(emp =>
				colabs.includes(emp.value) || colabs.includes(String(emp.value)),
			)

			const parentId = this.selectedClient.client_parent || 0
			this.client_parent = Number(parentId) === 0
				? null
		...
		async importar() {
			...
			formData.append('clientsfileXLSX', file)

			this.loading = true

			try {
				await axios.post(generateUrl('/apps/employees/importarClientes'), formData, {
					headers: { 'Content-Type': 'multipart/form-data' },
				})

				showSuccess(t('employees', 'Companies imported successfully'))
				await this.GetCompaniesGroups()
			} catch (err) {
				showError(t('employees', 'Error importing companies: {error}', { error: String(err) }))
			} finally {
				this.loading = false
		...
		async Exportar() {
			this.toggle()
			try {
				const response = await axios.get(generateUrl('/apps/employees/Exportarclients'), {
					responseType: 'blob',
				})
				...
				showError(t('employees', 'Error exporting companies: {error}', { error: String(err) }))
			}
		},
		...
		async GetHonorariosByCliente(idClient) {
			if (!idClient) {
				this.ProfessionalFee = []
				return
			}
			...
			try {
				const response = await axios.post(
					generateUrl('/apps/employees/findHonorariosByCliente'),
					{ id_client: idClient },
				)

				const data = this.getOcsData(response)
				this.ProfessionalFee = Array.isArray(data) ? data : []
			} catch (err) {
				showError(t('employees', 'Error loading fees: {error}', { error: String(err) }))
				this.ProfessionalFee = []
			} finally {
		...
		async crearHonorario() {
			...
			if (!this.isHonorarioValid) {
				showError(t('employees', 'Fill in amount, currency and dates.'))
				return
			}
			...
			try {
				...
				let endDate = ''
				if (this.h_tipo_honorario === 'parcial') {
					endDate = this.h_mes_fin && this.h_anio_fin
						? `${this.h_anio_fin.value}-${String(this.h_mes_fin.value).padStart(2, '0')}-01`
						: ''
				} else if (this.h_tipo_honorario === 'eventual') {
					// misma date que inicio
					endDate = this.h_mes_inicio && this.h_anio_inicio
						? `${this.h_anio_inicio.value}-${String(this.h_mes_inicio.value).padStart(2, '0')}-01`
						: ''
				}
				// iguala: date_end vacía (indefinida)

				const payload = {
					id_client: this.selectedClient.id,
					amount_total: Number(this.h_importe_total),
					type_currency: this.h_tipo_moneda?.value || 'MXN',
					special: this.h_especial ? 1 : 0,
					type_fee: this.h_tipo_honorario,
					date_start: this.h_mes_inicio && this.h_anio_inicio
						? `${this.h_anio_inicio.value}-${String(this.h_mes_inicio.value).padStart(2, '0')}-01`
						: '',
					date_end: endDate,
					service_type: (() => {
				...
				if (this.honorarioBorradorId) {
					await axios.post(generateUrl('/apps/employees/completarHonorario'), {
						...payload,
						id_fee: this.honorarioBorradorId,
					})
				} else {
					await axios.post(generateUrl('/apps/employees/crearHonorario'), payload)
				}

				showSuccess(t('employees', 'Service fee created successfully'))
				await this.GetHonorariosByCliente(this.selectedClient.id)
				this.closeHonorarioModal()
			} catch (err) {
				showError(t('employees', 'Error creating fee: {error}', { error: String(err) }))
			} finally {
				this.savingHonorario = false
			}
		},
		...
		async confirmDeleteHonorario() {
			...
			try {
				await axios.post(generateUrl('/apps/employees/deleteHonorario'), {
					id_fee: this.honorarioToDelete,
				})

				showSuccess(
					t('employees', 'Service fee deleted successfully'),
				)
				...
				showError(
					t('employees', 'Error deleting fee: {error}', {
						error: String(err),
					}),
				)
			} finally {
		...
		completarHonorarioBorrador(honorario) {
			...
			this.h_importe_total = String(honorario.amount_total)

			// Guardar referencia para saber que es edición, no creación
			this.honorarioBorradorId = honorario.id_fee
		...
		editarHonorario(honorario) {
			...
			this.h_importe_total = String(honorario.amount_total)
			this.h_tipo_moneda = this.currencyOptions.find(c => c.value === honorario.type_currency) || this.currencyOptions[0]
			this.h_tipo_servicio = honorario.service_type || ''

			// Parsear fechas a mes/año
			if (honorario.date_start) {
				const [anio, mes] = honorario.date_start.split('-')
				this.h_anio_inicio = { label: anio, value: Number(anio) }
				this.h_mes_inicio = this.monthOptions?.find(m => m.value === Number(mes)) || null
			}

			if (honorario.date_end) {
				const [anio, mes] = honorario.date_end.split('-')
		...
		async GetParcialidades(idHonorario) {
			...
			try {
				const response = await axios.post(
					generateUrl('/apps/employees/findParcialidadesByHonorario'),
					{ id_fee: idHonorario },
				)
				...
				showError(t('employees', 'Error loading installments: {error}', { error: String(err) }))
			} finally {
				this.loadingParcialidades = {
					...this.loadingParcialidades,
					[idHonorario]: false,
		...
		async confirmarPago() {
			try {
				await axios.post(
					generateUrl('/apps/employees/marcarParcialidadPagada'),
					{
						id_installment: this.parcialidadSeleccionada,
						date_payment: this.fechaPago,
					},
				)
				...
				showSuccess(
					t('employees', 'Installment marked as paid'),
				)
		...
		editarFechaPago(p, idHonorario) {
			this.parcialidadSeleccionada = p.id_installment
			this.honorarioSeleccionado = idHonorario
			this.fechaPago = p.date_payment || new Date().toISOString().split('T')[0]
			this.showPagoDialog = true
		},

		askCancelarPago(p, idHonorario) {
			this.parcialidadACancelar = p.id_installment
			this.honorarioSeleccionado = idHonorario
		...
		async confirmarCancelarPago() {
			try {
				await axios.post(
					generateUrl('/apps/employees/cancelarPagoParcialidad'),
					{ id_installment: this.parcialidadACancelar },
				)

				await this.GetParcialidades(this.honorarioSeleccionado)
				await this.GetHonorariosByCliente(this.selectedClient.id)

				showSuccess(t('employees', 'Payment cancelled'))
			} catch (err) {
				showError(t('employees', 'Error cancelling payment: {error}', { error: String(err) }))
			} finally {
		...
		async confirmarFactura(idParcialidad, idHonorario) {
			try {
				await axios.post(
					generateUrl('/apps/employees/mark-fee-payment-invoiced'),
					{
						id_installment: idParcialidad,
					},
				)
				...
				showSuccess(t('employees', 'Installment marked as invoiced'))
		...
		async toggleEstado() {
			...
			try {
				await axios.post(generateUrl('/apps/employees/modificarCliente'), {
					...
					legal_name: this.selectedClient.legal_name || '',
					project_leader: this.selectedClient.project_leader || null,
					collaborators: JSON.stringify(
						Array.isArray(this.selectedClient.collaborators)
							? this.selectedClient.collaborators
							: [],
					),
					name_contact: this.selectedClient.name_contact || '',
					phone: this.selectedClient.phone || '',
					email: this.selectedClient.email || '',
					location: this.selectedClient.location || '',
					details: this.selectedClient.details || '',
					special: Number(this.selectedClient.special) || 0,
					client_parent: this.selectedClient.client_parent || null,
					status: this.selectedIsActive ? 0 : 1,
				})

				showSuccess(
					this.selectedIsActive
						? t('employees', 'Company disabled successfully')
						: t('employees', 'Company enabled successfully'),
				)

				await this.GetCompaniesGroups()
				await this.GetCompanieGroup(this.selectedClient.id)
			} catch (err) {
				showError(t('employees', 'Error updating status: {error}', { error: String(err) }))
			}
		},
		...
		generarReporteHonorarios() {
			...
			showSuccess(
				t('employees', '{n} fees selected for report', { n: this.selectedHonorarios.length }),
			)
		},
		...
		async agregarParcialidadIguala(idHonorario) {
			try {
				await axios.post(
					generateUrl('/apps/employees/agregarParcialidadIguala'),
					{ id_fee: idHonorario },
				)
				await this.GetParcialidades(idHonorario)
				showSuccess(t('employees', 'Installment added'))
			} catch (err) {
				showError(t('employees', 'Error adding installment: {error}', { error: String(err) }))
			}
		},
		...
		async confirmarFinalizarHonorario() {
			try {
				await axios.post(
					generateUrl('/apps/employees/finalizarHonorario'),
					{ id_fee: this.honorarioToFinalizar },
				)
				await this.GetHonorariosByCliente(this.selectedClient.id)
				showSuccess(t('employees', 'Fee finalized'))
			} catch (err) {
				showError(t('employees', 'Error finalizing fee: {error}', { error: String(err) }))
			} finally {
		...
		async reactivarHonorario(idHonorario) {
			try {
				await axios.post(
					generateUrl('/apps/employees/reactivarHonorario'),
					{ id_fee: idHonorario },
				)
				await this.GetHonorariosByCliente(this.selectedClient.id)
				showSuccess(t('employees', 'Fee reactivated'))
			} catch (err) {
				showError(t('employees', 'Error reactivating fee: {error}', { error: String(err) }))
			}
		},
		...
		abrirModificarHonorario(honorario) {
			...
			this.editingHonorarioId = honorario.id_fee
			this.h_tipo_honorario = honorario.type_fee || 'parcial'
			this.h_especial = Boolean(Number(honorario.special))
			this.h_importe_total = String(honorario.amount_total)

			this.h_tipo_moneda = this.currencyOptions.find(c => c.value === honorario.type_currency)
				|| this.currencyOptions[0]

			// Separar el service_type base del sufijo "- Mes Año" que se concatena al crear
			const raw = String(honorario.service_type || '')
		...
		async guardarModificacionHonorario() {
			...
			try {
				...
				await axios.post(generateUrl('/apps/employees/actualizarMetadatosHonorario'), {
					id_fee: this.editingHonorarioId,
					service_type: tipoServicioFinal,
					type_currency: this.h_tipo_moneda?.value || 'MXN',
					special: this.h_especial ? 1 : 0,
				})

				showSuccess(t('employees', 'Service fee updated successfully'))
				await this.GetHonorariosByCliente(this.selectedClient.id)
				this.closeHonorarioModal()
			} catch (err) {
				showError(t('employees', 'Error updating fee: {error}', { error: String(err) }))
			} finally {
				this.savingHonorario = false
			}
		},
		...
		async abrirReporteHonorario(honorario) {
			...
			this.rep_asunto = honorario.service_type || ''
		...
		async generarReporte() {
			if (!this.honorarioParaReporte?.id_fee) {
				showError(t('employees', 'No fee selected for the report.'))
				return
			}
			...
			try {
				const response = await axios.get(
					generateUrl('/apps/employees/generarSolicitudRecibo'),
					{
						responseType: 'blob',
						params: {
							id_fee: this.honorarioParaReporte.id_fee,
				...
				const nombreCliente = (this.selectedClient?.name || 'cliente')
				...
				showError(t('employees', 'Error generating report: {error}', { error: String(err) }))
			} finally {
...
.date-pago-input:focus {
	outline: none;
	border-color: var(--color-primary);
	box-shadow: 0 0 0 2px var(--color-primary-light);
}

.date-pago-input:hover {
	border-color: var(--color-primary);
}
