...
export function normalizeTechnicians(items) {
	...
	for (const item of items) {
		...
		if (!uid) continue
		const displayName = String(item?.displayName ?? item?.displayname ?? item?.label ?? item?.name ?? item?.name ?? uid).trim() || uid
...
export default {
	...
	getGroupMaintenances(id, filters = {}, signal) { return request('get', `/inventario/mantenimientos/grupos/${id}/Team`, { params: filters, signal }) },
	...
	getEligibleEquipment(departmentId, filters = {}, signal) { return request('get', `/inventario/mantenimientos/Department/${departmentId}/Team`, { params: filters, signal }) },
	...
	getEquipmentHistory(id, filters = {}, signal) { return request('get', `/inventario/mantenimientos/Team/${id}/historial`, { params: filters, signal }) },
