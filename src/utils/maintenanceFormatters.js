...
export function maintenanceRecordCapabilities(maintenance = {}, capabilities = {}, currentUid = '') {
	const status = String(maintenance.status || '')
	...
	const assignedTechnician = capabilities.isTechnician === true
		&& String(currentUid || '') !== ''
		&& String(currentUid) === String(maintenance.technician_uid || '')
	...
	return {
		...
		canReschedule: canOperate && ['pending', 'scheduled', 'in_progress'].includes(status) && Boolean(maintenance.date_scheduled),
