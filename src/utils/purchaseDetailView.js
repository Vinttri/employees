...
export function getPurchaseDetailActions({ request = {}, permissions = {}, flow = null, currentUserId = '' } = {}) {
	const state = String(request.status || '')
	const isOwner = String(request.id_user || '') === String(currentUserId || '')
	...
	return {
		exportPdf: Boolean(request.id_request),
