...
class MaintenanceController extends BaseController {
	...
	public function createGroup(mixed $title = null, mixed $departmentId = null, mixed $includeDescendants = false, mixed $type = null, mixed $periodStart = null, mixed $periodEnd = null, mixed $startTime = null, mixed $endTime = null, mixed $technicianUid = null, mixed $description = null, mixed $equipmentIds = [], mixed $allowPotentialDuplicates = false, mixed $scheduledDate = null): DataResponse {
		return $this->execute(function (array $actor) use ($title, $departmentId, $includeDescendants, $type, $periodStart, $periodEnd, $startTime, $endTime, $technicianUid, $description, $equipmentIds, $allowPotentialDuplicates, $scheduledDate): array {
			...
			return $this->maintenanceService->createGroup([
				'title' => $this->normalizeRequiredString($title, 255, 'El título'),
				'id_department' => $this->normalizeNullableId($departmentId),
				'include_descendants' => $this->normalizeBoolean($includeDescendants),
				'type' => $this->normalizeRequiredString($type, 40, 'El type'),
				'date_start' => $this->normalizeDate($normalizedPeriodStart, 'La date inicial del periodo'),
				'date_end' => $this->normalizeDate($normalizedPeriodEnd, 'La date final del periodo'),
				'time_start' => $this->normalizeNullableTime($startTime),
				'time_end' => $this->normalizeNullableTime($endTime),
				'technician_uid' => $technicianUid,
				'description' => $this->normalizeNullableString($description, 65535),
	...
	public function cancelGroup(mixed $id, mixed $reason = null): DataResponse {
		return $this->execute(function (array $actor) use ($id, $reason): array {
			...
			return $this->maintenanceService->cancelGroup(
				$this->normalizePositiveId($id, 'grupo'), $this->normalizeRequiredString($reason, 4000, 'El reason'),
	...
	public function schedule(mixed $id, mixed $scheduledDate = null, mixed $startTime = null, mixed $endTime = null): DataResponse {
		return $this->maintenanceWrite($id, 'schedule', fn(int $maintenanceId, array $actor): array => $this->maintenanceService->scheduleMaintenance(
			$maintenanceId, $this->normalizeDate($scheduledDate, 'La date programada'),
	...
	#[UseSession]
	#[NoAdminRequired]
	public function complete(mixed $id, mixed $result = null, mixed $actionsPerformed = null, mixed $incidents = null, mixed $spare_parts = null, mixed $observations = null, mixed $nextDate = null): DataResponse {
		return $this->maintenanceWrite($id, 'complete', function (int $maintenanceId, array $actor) use ($result, $actionsPerformed, $incidents, $spare_parts, $observations, $nextDate): array {
			return $this->maintenanceService->completeMaintenance($maintenanceId, $this->normalizeWorkDetails([
				'result' => $result, 'actions_performed' => $actionsPerformed,
				'incidents' => $incidents, 'spare_parts' => $spare_parts,
				'observations' => $observations, 'next_date' => $nextDate,
	...
	public function reschedule(mixed $id, mixed $scheduledDate = null, mixed $startTime = null, mixed $endTime = null, mixed $reason = null): DataResponse {
		return $this->maintenanceWrite($id, 'reschedule', fn(int $maintenanceId, array $actor): array => $this->maintenanceService->rescheduleMaintenance(
			$maintenanceId, $this->normalizeDate($scheduledDate, 'La date programada'),
			$this->normalizeNullableTime($startTime), $this->normalizeNullableTime($endTime),
			$this->normalizeRequiredString($reason, 4000, 'El reason'), $actor['uid'], $actor['displayName'],
		));
	}
	...
	public function cancel(mixed $id, mixed $reason = null): DataResponse {
		return $this->execute(function (array $actor) use ($id, $reason): array {
			...
			$maintenance = $this->maintenanceService->cancelMaintenance(
				$this->normalizePositiveId($id, 'mantenimiento'), $this->normalizeRequiredString($reason, 4000, 'El reason'),
				$actor['uid'], $actor['displayName'],
			);
	...
	public function notApplicable(mixed $id, mixed $reason = null): DataResponse {
		return $this->maintenanceWrite($id, 'not_applicable', fn(int $maintenanceId, array $actor): array => $this->maintenanceService->markNotApplicable(
			$maintenanceId, $this->normalizeRequiredString($reason, 4000, 'El reason'), $actor['uid'], $actor['displayName'],
		));
	}
	...
	public function updateChecklist(mixed $id, mixed $items = []): DataResponse {
		return $this->execute(function (array $actor) use ($id, $items): array {
			...
			if (!is_array($items)) throw new MaintenanceValidationException('El checklist debe ser una lista.');
			$normalized = [];
			foreach ($items as $item) {
				if (!is_array($item)) throw new MaintenanceValidationException('Cada elemento del checklist debe ser un objeto.');
				$normalized[] = [
					'code' => $this->normalizeRequiredString($item['key'] ?? null, 80, 'La code'),
					'result' => $this->normalizeRequiredString($item['result'] ?? null, 24, 'El result'),
					'observation' => $this->normalizeNullableString($item['observation'] ?? null, 4000),
				];
			}
	...
	#[UseSession]
	#[NoAdminRequired]
	public function updateWork(mixed $id, mixed $result = null, mixed $actionsPerformed = null, mixed $incidents = null, mixed $spare_parts = null, mixed $observations = null, mixed $nextDate = null): DataResponse {
		return $this->maintenanceWrite($id, 'update_work', function (int $maintenanceId, array $actor) use ($result, $actionsPerformed, $incidents, $spare_parts, $observations, $nextDate): array {
	...
	public function duplicates(mixed $equipmentIds = [], mixed $type = null, mixed $periodStart = null, mixed $periodEnd = null, mixed $scheduledDate = null): DataResponse {
		return $this->execute(function (array $actor) use ($equipmentIds, $type, $periodStart, $periodEnd, $scheduledDate): array {
			...
			$ids = $this->normalizeIdArray($equipmentIds);
			$type = $this->normalizeRequiredString($type, 40, 'El type');
			$from = $this->normalizeDate($periodStart ?? $scheduledDate, 'La date inicial del periodo');
			$to = $this->normalizeDate($periodEnd ?? $scheduledDate, 'La date final del periodo');
			$items = [];
	...
	private function execute(callable $operation, string $operationName, int $successStatus = Http::STATUS_OK, array $context = []): DataResponse {
		...
		} catch (MaintenanceTransitionException $e) {
		...
		} catch (MaintenanceStorageException $e) {
	...
	private function requireEligibleTechnician(?string $uid): void {
		...
		if (!$this->permissions->canManageInventory($uid) && !$this->permissions->canSee('inventario.technician', $uid)) {
			throw new MaintenanceValidationException('El usuario seleccionado no es un técnico de inventario autorizado.');
		}
	}
	...
	private function requireMaintenanceReadAccess(array $maintenance, string $uid): string {
		$role = $this->requireReadRole($uid);
		if ($role === 'technician' && (string)($maintenance['technician_uid'] ?? '') !== $uid) $this->deny('No tienes acceso a este mantenimiento.');
		return $role;
	}
	...
	private function requireMaintenanceWriteAccess(array $maintenance, string $uid): void {
		...
		if ($role !== 'technician' || (string)($maintenance['technician_uid'] ?? '') !== $uid) $this->deny('No tienes permiso para modificar este mantenimiento.');
	}
	...
	private function withProgress(array $maintenance): array {
		return ['maintenance' => $maintenance, 'groupProgress' => $this->maintenanceService->getGroupProgress((int)$maintenance['id_group'])];
	}
	...
	private function normalizePositiveId(mixed $value, string $label): int {
		...
		else throw new MaintenanceValidationException('El identificador de ' . $label . ' no es válido.');
		if ($id <= 0) throw new MaintenanceValidationException('El identificador de ' . $label . ' no es válido.');
		return $id;
	}
	...
	private function normalizeBoolean(mixed $value): bool {
		...
		throw new MaintenanceValidationException('El valor booleano no es válido.');
	}
	...
	private function normalizeLimit(mixed $value): int {
		...
		if ($limit > self::MAX_LIMIT) throw new MaintenanceValidationException('El límite máximo es ' . self::MAX_LIMIT . '.');
		return $limit;
	}
	...
	private function normalizeIdArray(mixed $values): array {
		if (!is_array($values) || $values === []) throw new MaintenanceValidationException('Debe seleccionar al menos un equipo.');
		if (count($values) > self::MAX_EQUIPMENT_IDS) throw new MaintenanceValidationException('No se permiten más de ' . self::MAX_EQUIPMENT_IDS . ' Team por campaña.');
	...
	private function normalizeDate(mixed $value, string $label): string {
		if (!is_string($value) || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $value)) throw new MaintenanceValidationException($label . ' no es válida.');
		$date = \DateTimeImmutable::createFromFormat('!Y-m-d', $value);
		$errors = \DateTimeImmutable::getLastErrors();
		if ($date === false || ($errors !== false && ($errors['warning_count'] > 0 || $errors['error_count'] > 0)) || $date->format('Y-m-d') !== $value) throw new MaintenanceValidationException($label . ' no es válida.');
		return $value;
	}
	...
	private function normalizeDateRange(mixed $start, mixed $end, bool $required): array {
		...
		if ($start === null || $start === '' || $end === null || $end === '') throw new MaintenanceValidationException('Las fechas inicial y final son obligatorias.');
		$from = $this->normalizeDate($start, 'La date inicial');
		$to = $this->normalizeDate($end, 'La date final');
		...
		if ($days > self::MAX_CALENDAR_DAYS) throw new MaintenanceValidationException('El rango no puede exceder ' . self::MAX_CALENDAR_DAYS . ' días.');
		return [$from, $to];
	}
	...
	private function normalizeNullableTime(mixed $value): ?string {
		...
		if ($value !== null && !preg_match('/^(?:[01]\d|2[0-3]):[0-5]\d$/', $value)) throw new MaintenanceValidationException('La hora no tiene formato HH:MM válido.');
		return $value;
	}

	private function normalizeGroupStatus(mixed $value): ?string {
		$status = $this->normalizeNullableString($value, 20);
		if ($status !== null && !in_array($status, MaintenanceGroup::ESTADOS_VALIDOS, true)) throw new MaintenanceValidationException('El status de campaña no es válido.');
		return $status;
	}

	private function normalizeMaintenanceStatus(mixed $value): ?string {
		$status = $this->normalizeNullableString($value, 24);
		if ($status !== null && !in_array($status, MaintenanceAsset::ESTADOS_VALIDOS, true)) throw new MaintenanceValidationException('El status de mantenimiento no es válido.');
		return $status;
	}
	...
	private function normalizeRequiredString(mixed $value, int $maxLength, string $label): string {
		...
		if ($value === null) throw new MaintenanceValidationException($label . ' es obligatorio.');
		return $value;
	}

	private function normalizeNullableString(mixed $value, int $maxLength): ?string {
		if ($value === null) return null;
		if (!is_scalar($value)) throw new MaintenanceValidationException('Se recibió un texto no válido.');
		$value = trim((string)$value);
		if ($value === '') return null;
		if (mb_strlen($value) > $maxLength) throw new MaintenanceValidationException('Un texto excede la longitud permitida.');
		return $value;
	}
	...
	private function normalizeWorkDetails(array $values, bool $required): array {
		...
		foreach ($values as $key => $value) {
			if ($key === 'next_date') {
				...
				if ($normalized !== null) $normalized = $this->normalizeDate($normalized, 'La próxima date');
			} else {
				$normalized = $this->normalizeNullableString($value, 65535);
			}
			if ($normalized !== null || ($required && in_array($key, ['result', 'actions_performed'], true))) $result[$key] = $normalized;
		}
		return $result;
	}

	private function sanitizeGroupSummary(array $group): array {
		unset($group['description'], $group['created_by']);
		return $group;
	}

	private function sanitizeMaintenanceSummary(array $maintenance): array {
		foreach (['result', 'actions_performed', 'incidents', 'spare_parts', 'observations', 'employee_name'] as $field) unset($maintenance[$field]);
		return $maintenance;
	}
	...
	private function sanitizeReadOnlyDetail(array $detail): array {
		...
		foreach (['employee_uid', 'technician_uid', 'technician_name', 'serial_number', 'updated_by', 'created_by'] as $field) unset($detail['maintenance'][$field]);
		$detail['audit'] = [];
		foreach ($detail['checklist'] as &$item) unset($item['observation'], $item['updated_by']);
		unset($item);
		return $detail;
	}
	...
	private function normalizeEligibleEquipment(array $row): array {
		return [
			'id' => (int)$row['id_team'],
			'identifier' => $row['system_name'] ?: $row['device_name'],
			'name' => $row['device_name'],
			'model' => $row['model'], 'brand' => $row['brand'], 'serialNumber' => $row['serial_number'], 'status' => $row['status'],
			'employee' => $row['id_employee'] === null ? null : ['id' => (int)$row['id_employee'], 'uid' => $row['employee_uid'], 'name' => $row['employee_name']],
			'department' => ['id' => (int)$row['id_department'], 'name' => $row['department_name']],
